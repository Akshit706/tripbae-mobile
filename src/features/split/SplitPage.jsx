  ...
  return (
    <div style={{ padding: '0 1.25rem' }}>
      ...
      <div style={{ background: 'linear-gradient(135deg,#7A2800,#FF6A00 50%,#FF8C3A)', borderRadius: 20, padding: '1.4rem 1.6rem', marginBottom: '1.25rem', position: 'relative', overflow: 'hidden', boxShadow: '0 10px 40px rgba(255,106,0,0.38)', borderTop: '0.5px solid rgba(255,255,255,0.28)' }}>
        ...
        {/* ⓘ Lumi info button */}
        <button onClick={() => setShowWelcome(true)} title="About Split" style={{ position:'absolute', top:10, right:10, width:26, height:26, borderRadius:'50%', border:'1.5px solid rgba(255,255,255,0.35)', background:'rgba(255,255,255,0.15)', backdropFilter:'blur(6px)', cursor:'pointer', display:'flex', alignItems:'center', justifyContent:'center', zIndex:5, padding:0 }}>
            ...
            return (
              <div key={m} style={{ background: '#fff', border: '1px solid rgba(0,0,0,0.07)', borderRadius: 16, padding: '13px 14px', marginBottom: 8, boxShadow: '0 2px 10px rgba(0,0,0,0.04)', animation: `soloFadeUp .35s ease-out ${i * 55}ms both` }}>
                <div style={{ display: 'flex', alignItems: 'center', gap: 10, marginBottom: 9 }}>
                  ...
                  <div style={{ textAlign: 'right', flexShrink: 0 }}>
                    <div style={{ fontFamily: "'Sora',sans-serif", fontSize: 16, fontWeight: 800, color: isPos ? '#FF6A00' : isNeg ? '#D85B00' : '#6b7280' }}>{isPos ? '+' : isNeg ? '−' : ''}₹{Math.abs(Math.round(net)).toLocaleString('en-IN')}</div>
