...
/* ─── BRAND CONSTANTS ───────────────────────────────── */
// TripBae brand colors from the brand kit
const BRAND = {
  purple: '#7B61FF',
  pink: '#FF6AAE',
  orange: '#FF8A3D',
  ...
  textMuted: '#8B93B5',
  textDim: '#4A5280',
  gradient: 'linear-gradient(135deg, #7B61FF 0%, #FF6AAE 50%, #FF8A3D 100%)',
  gradientSubtle: 'linear-gradient(135deg, rgba(123,97,255,0.15), rgba(255,106,174,0.10))',
  gradientBtn: 'linear-gradient(135deg, #7B61FF, #FF6AAE)',
  success: '#00D4A0',
  successBg: 'rgba(0,212,160,0.1)',
  successBorder: 'rgba(0,212,160,0.25)',
  danger: '#FF4D6A',
  dangerBg: 'rgba(255,77,106,0.1)',
  dangerBorder: 'rgba(255,77,106,0.25)',
  yellow: '#FFB703',
};

const MCOLORS = [BRAND.purple,'#FF6AAE','#FF8A3D','#00D4A0','#378ADD','#FF4D6A','#7B61FF','#FFB703'];

const CATS = [
  {id:'food',icon:'🍽️',label:'Food',bg:'rgba(255,183,3,0.12)',color:BRAND.yellow},
  {id:'transport',icon:'🚗',label:'Transport',bg:'rgba(0,212,160,0.12)',color:BRAND.success},
  {id:'stay',icon:'🏠',label:'Stay',bg:'rgba(55,138,221,0.12)',color:'#378ADD'},
  {id:'activity',icon:'🎟️',label:'Activity',bg:'rgba(123,97,255,0.12)',color:BRAND.purple},
  {id:'shopping',icon:'🛍️',label:'Shopping',bg:'rgba(255,106,174,0.12)',color:BRAND.pink},
];
const CONTACT_CATS = [
  {id:'guardian',icon:'🛡️',label:'Guardian',bg:'rgba(123,97,255,0.12)',color:BRAND.purple},
  {id:'driver',icon:'🚗',label:'Driver',bg:'rgba(0,212,160,0.12)',color:BRAND.success},
  {id:'hotel',icon:'🏨',label:'Hotel Staff',bg:'rgba(55,138,221,0.12)',color:'#378ADD'},
  {id:'guide',icon:'🗺️',label:'Guide',bg:'rgba(255,183,3,0.12)',color:BRAND.yellow},
  {id:'medical',icon:'🏥',label:'Medical',bg:'rgba(255,77,106,0.12)',color:BRAND.danger},
  {id:'emergency',icon:'🚨',label:'Emergency',bg:'rgba(255,138,61,0.12)',color:BRAND.orange},
  {id:'other',icon:'👤',label:'Other',bg:'rgba(139,147,181,0.12)',color:BRAND.textMuted},
];
const INTERESTS = ['🏖️ Beaches','🛕 Temples','🌿 Nature','🍽️ Food','🧗 Adventure','🎭 Culture','🛍️ Shopping','🌙 Nightlife','🏛️ History','💆 Wellness'];
...
/* ─── LOGO SVG ───────────────────────────────────────── */
function TripBaeLogo({ size = 32 }) {
  return (
    <svg width={size} height={size} viewBox="0 0 40 40" fill="none" xmlns="http://www.w3.org/2000/svg">
      <defs>
        <linearGradient id="logoGrad" x1="0%" y1="0%" x2="100%" y2="100%">
          <stop offset="0%" stopColor="#7B61FF"/>
          <stop offset="50%" stopColor="#FF6AAE"/>
          <stop offset="100%" stopColor="#FF8A3D"/>
        </linearGradient>
      </defs>
      {/* Infinity loop */}
      <path d="M6 20 C6 14 10 10 15 10 C19 10 21 13 24 17 C27 21 29 24 33 24 C36 24 34 18 33 17 M34 20 C34 26 30 30 25 30 C21 30 19 27 16 23 C13 19 11 16 7 16 C4 16 6 22 7 23" 
        stroke="url(#logoGrad)" strokeWidth="3.5" strokeLinecap="round" fill="none"/>
      {/* Airplane */}
      <text x="27" y="11" fontSize="9" fill="url(#logoGrad)">✈</text>
    </svg>
  );
}
...
function WordMark({ size = 'md' }) {
  ...
  const fs = sizes[size] || 19;
  return (
    <span style={{ fontFamily: "'Poppins', 'DM Sans', sans-serif", fontSize: fs, fontWeight: 700, letterSpacing: '-0.3px', color: BRAND.text }}>
      trip<span style={{ background: BRAND.gradient, WebkitBackgroundClip: 'text', WebkitTextFillColor: 'transparent', backgroundClip: 'text' }}>bae</span>
    </span>
  );
}
...
function Spinner({ text, solo }) {
  return (
    <div style={{ textAlign: 'center', padding: '3rem 1rem' }}>
      <div style={S.spinner} />
      <p style={{ fontSize: 14, color: BRAND.textMuted, marginTop: 12 }}>{text || 'Loading…'}</p>
    </div>
  );
}
...
function Stars({ n, rating }) {
  return (
    <span>
      {Array.from({ length: 5 }, (_, i) => (
        <span key={i} style={{ color: i < n ? BRAND.yellow : BRAND.textDim, fontSize: 11 }}>★</span>
      ))}
      <span style={{ fontSize: 11, color: BRAND.textMuted, marginLeft: 4 }}>{rating}</span>
    </span>
  );
}
...
function tripStatusInfo(arrival, departure, completed) {
  if (completed) {
    return { label: 'Completed', color: BRAND.textMuted, bg: 'rgba(139,147,181,0.12)', border: BRAND.border, isPast: true };
  }
  const now = new Date(); const a = new Date(arrival); const d = new Date(departure);
  if (now < a) {
    const daysLeft = Math.ceil((a - now) / 86400000);
    return { label: `In ${daysLeft}d`, color: BRAND.success, bg: BRAND.successBg, border: BRAND.successBorder, isPast: false };
  } else if (now <= d) {
    return { label: 'Ongoing', color: BRAND.orange, bg: 'rgba(255,138,61,0.12)', border: 'rgba(255,138,61,0.3)', isPast: false };
  }
  return { label: 'Past', color: BRAND.textMuted, bg: 'rgba(139,147,181,0.12)', border: BRAND.border, isPast: false };
}
...
function ConfirmDialog({ title, message, confirmLabel, confirmStyle, onConfirm, onCancel }) {
  return (
    <div style={{ position: 'fixed', inset: 0, background: 'rgba(5,8,20,0.85)', backdropFilter: 'blur(12px)', zIndex: 600, display: 'flex', alignItems: 'center', justifyContent: 'center', padding: '1.5rem' }}>
      <div style={{ background: BRAND.navyCard, border: `1px solid ${BRAND.border}`, borderRadius: 20, padding: '1.75rem', maxWidth: 340, width: '100%', boxShadow: '0 24px 80px rgba(0,0,0,0.6)', textAlign: 'center' }}>
        <div style={{ fontSize: 42, marginBottom: 12 }}>{confirmStyle === 'danger' ? '🗑️' : '✅'}</div>
        <div style={{ fontFamily: "'Poppins',sans-serif", fontSize: 17, fontWeight: 700, marginBottom: 8, color: BRAND.text }}>{title}</div>
        <div style={{ fontSize: 13, color: BRAND.textMuted, lineHeight: 1.6, marginBottom: 22 }}>{message}</div>
        <div style={{ display: 'flex', gap: 10 }}>
          <button style={{ ...S.btn, flex: 1, justifyContent: 'center', padding: '11px' }} onClick={onCancel}>Cancel</button>
          <button
            style={{ ...S.btn, flex: 1, justifyContent: 'center', padding: '11px', fontWeight: 600,
              ...(confirmStyle === 'danger' ? { background: BRAND.dangerBg, color: BRAND.danger, border: `1px solid ${BRAND.dangerBorder}` } : S.btnP) }}
            onClick={onConfirm}>
            {confirmLabel}
          </button>
        </div>
...
/* ─── STYLES ─────────────────────────────────────────── */
const S = {
  root: { fontFamily: "'DM Sans', 'Poppins', sans-serif", background: BRAND.navy, color: BRAND.text, minHeight: '100vh', WebkitFontSmoothing: 'antialiased' },
  topBar: { background: 'rgba(13,19,41,0.95)', borderBottom: `1px solid ${BRAND.borderLight}`, padding: '0 1.25rem', display: 'flex', alignItems: 'center', gap: 12, position: 'sticky', top: 0, zIndex: 200, backdropFilter: 'blur(20px)', boxShadow: '0 1px 0 rgba(123,97,255,0.1)' },
  navTabs: { background: 'rgba(13,19,41,0.95)', borderBottom: `1px solid ${BRAND.borderLight}`, display: 'flex', padding: '0 1rem', overflowX: 'auto', backdropFilter: 'blur(20px)' },
  navTab: { display: 'flex', alignItems: 'center', gap: 5, padding: '12px 12px', fontSize: 12, fontWeight: 400, color: BRAND.textMuted, borderBottom: '2px solid transparent', cursor: 'pointer', background: 'none', border: 'none', fontFamily: "'DM Sans',sans-serif", whiteSpace: 'nowrap' },
  navTabActive: { color: BRAND.purple, borderBottom: `2px solid ${BRAND.purple}`, fontWeight: 600 },
  soloNavTabActive: { color: BRAND.pink, borderBottom: `2px solid ${BRAND.pink}`, fontWeight: 600 },
  page: { padding: '1.25rem', flex: 1, paddingBottom: '6rem' },
  btn: { display: 'inline-flex', alignItems: 'center', gap: 6, padding: '7px 13px', borderRadius: 10, fontSize: 13, fontWeight: 500, border: `1px solid ${BRAND.borderLight}`, background: BRAND.navyLight, color: BRAND.text, cursor: 'pointer', fontFamily: "'DM Sans',sans-serif", transition: 'all .15s' },
  btnP: { background: BRAND.gradientBtn, color: '#fff', border: 'none', boxShadow: '0 4px 16px rgba(123,97,255,0.35)' },
  btnSolo: { background: 'linear-gradient(135deg,#FF6AAE,#FF8A3D)', color: '#fff', border: 'none', boxShadow: '0 4px 16px rgba(255,106,174,0.35)' },
  btnOutline: { background: 'transparent', color: BRAND.purple, border: `1px solid ${BRAND.border}` },
  btnDanger: { background: BRAND.dangerBg, color: BRAND.danger, border: `1px solid ${BRAND.dangerBorder}` },
  card: { background: BRAND.navyCard, border: `1px solid ${BRAND.borderLight}`, borderRadius: 14, padding: '1rem 1.1rem', marginBottom: 10 },
  input: { fontFamily: "'DM Sans',sans-serif", padding: '9px 12px', border: `1px solid ${BRAND.borderLight}`, borderRadius: 10, fontSize: 14, background: BRAND.navyLight, color: BRAND.text, width: '100%', outline: 'none', boxSizing: 'border-box' },
  label: { fontSize: 11, color: BRAND.textMuted, fontWeight: 600, letterSpacing: .3, textTransform: 'uppercase', display: 'block', marginBottom: 5, marginTop: 10 },
  spinner: { width: 36, height: 36, border: `3px solid rgba(123,97,255,0.15)`, borderTopColor: BRAND.purple, borderRadius: '50%', animation: 'spin .75s linear infinite', margin: '0 auto 12px' },
};
...
function HomePage({ trips, onOpenTrip, onCreateTrip, onJoinTrip, onDeleteTrip, onMarkComplete, onMarkActive }) {
  ...
  const searchDest = useCallback(async (text) => {
    ...
    setDestLoading(true);
    try {
      const res = await fetch(`https://nominatim.openstreetmap.org/search?q=${encodeURIComponent(text)}&format=json&addressdetails=1&limit=7&accept-language=en`, { headers: { 'User-Agent': 'TripBae/1.0', 'Accept-Language': 'en' } });
      const data = await res.json();
      const TYPES = ['city','town','village','suburb','county','state','district','region'];
      const seen = new Set();
      const filtered = data.filter(p => {
  ...
  const handleCreate = async () => {
    ...
    setCreating(true);
    try {
      await onCreateTrip({ groupName: form.groupName, destination: form.destination, emoji: form.emoji, arrival: form.arrival, departure: form.departure, arrivalSlot: form.arrivalSlot, departureSlot: form.departureSlot, isSolo: isSoloMode, people: isSoloMode ? 1 : parseInt(form.people), budget: form.budget ? parseFloat(form.budget) : null, nickname: form.createdBy });
      setShowCreate(false);
      setForm({ groupName: '', destination: '', emoji: '✈️', arrival: today, departure: '', people: 2, createdBy: '', budget: '' });
    } catch (err) { alert('Could not create trip: ' + err.message); }
    setCreating(false);
  };
  ...
  const handleJoin = async () => {
    ...
    try {
      const result = await onJoinTrip(joinCode.trim().toUpperCase(), joinName.trim());
      setJoinSuccess(result); setJoinCode(''); setJoinName('');
    } catch (err) { setJoinError(err.message || 'Invalid code. Please check and try again.'); }
    setJoining(false);
  };
  ...
  if (showPast) {
    return (
      <div>
        {confirmDelete && (
          <ConfirmDialog title="Delete Trip" message={`Are you sure you want to delete "${confirmDelete.groupName}"? This cannot be undone.`} confirmLabel="🗑️ Delete" confirmStyle="danger" onConfirm={() => { onDeleteTrip(confirmDelete.id); setConfirmDelete(null); }} onCancel={() => setConfirmDelete(null)} />
        )}
        <div style={{ display: 'flex', alignItems: 'center', gap: 10, marginBottom: '1.25rem' }}>
          <button style={S.btn} onClick={() => setShowPast(false)}>← Back</button>
          <div style={{ fontFamily: "'Poppins',sans-serif", fontSize: 18, fontWeight: 700 }}>Past Trips</div>
          <span style={{ fontSize: 12, color: BRAND.textMuted, background: BRAND.navyLight, border: `1px solid ${BRAND.borderLight}`, borderRadius: 10, padding: '3px 10px' }}>{pastTrips.length} trip{pastTrips.length !== 1 ? 's' : ''}</span>
        </div>
        {pastTrips.length === 0 && (
          <div style={{ textAlign: 'center', padding: '3rem', color: BRAND.textMuted }}>
            <div style={{ fontSize: 48, marginBottom: 12 }}>🗂️</div>
            <p>No completed trips yet.</p>
          </div>
        )}
        {pastTrips.map(trip => {
          ...
          return (
            <div key={trip.id} style={{ ...S.card, padding: 0, overflow: 'hidden', marginBottom: 14 }}>
              <div style={{ position: 'relative', height: 90, overflow: 'hidden', borderRadius: '14px 14px 0 0', cursor: 'pointer' }} onClick={() => { setShowPast(false); onOpenTrip(trip.id); }}>
                {trip.coverUrl && <img src={trip.coverUrl} alt="" style={{ width: '100%', height: '100%', objectFit: 'cover', filter: 'grayscale(30%) brightness(0.5)' }} onError={e => e.target.style.display = 'none'} />}
                <div style={{ position: 'absolute', inset: 0, background: 'linear-gradient(180deg,rgba(0,0,0,0.1) 0%,rgba(0,0,0,0.7) 100%)' }} />
                <div style={{ position: 'absolute', top: 10, left: 12, fontSize: 24 }}>{trip.emoji}</div>
                <div style={{ position: 'absolute', top: 9, right: 11, display: 'flex', gap: 6 }}>
                  {trip.isSolo && <span style={{ fontSize: 10, fontWeight: 700, padding: '2px 9px', borderRadius: 10, background: 'rgba(255,106,174,0.2)', color: BRAND.pink, border: `1px solid rgba(255,106,174,0.3)` }}>Solo</span>}
                  <span style={{ fontSize: 10, fontWeight: 600, padding: '2px 9px', borderRadius: 10, background: 'rgba(139,147,181,0.2)', color: BRAND.textMuted }}>Completed</span>
                </div>
                <div style={{ position: 'absolute', bottom: 10, left: 12 }}>
                  <div style={{ fontFamily: "'Poppins',sans-serif", fontSize: 15, fontWeight: 700, color: '#fff' }}>{trip.groupName}</div>
                  <div style={{ fontSize: 11, color: 'rgba(255,255,255,0.7)' }}>📍 {trip.destination}</div>
                </div>
              </div>
              <div style={{ padding: '10px 14px', display: 'flex', gap: 14, flexWrap: 'wrap', alignItems: 'center' }}>
                {[['📅', formatDateRange(trip.arrival, trip.departure)], ['🌙', `${days} nights`], ['💰', `₹${Math.round(totalSpend).toLocaleString('en-IN')}`]].map(([icon, val]) => (
                  <div key={val} style={{ display: 'flex', alignItems: 'center', gap: 5, fontSize: 12, color: BRAND.textMuted }}><span>{icon}</span><span>{val}</span></div>
                ))}
                <div style={{ marginLeft: 'auto', display: 'flex', gap: 8, alignItems: 'center' }}>
                  <button onClick={() => onMarkActive(trip.id)} style={{ ...S.btn, fontSize: 11, padding: '4px 10px', color: BRAND.success, borderColor: BRAND.successBorder, background: BRAND.successBg }}>↩ Restore</button>
                  <button onClick={() => setConfirmDelete(trip)} style={{ ...S.btn, fontSize: 11, padding: '4px 10px', ...S.btnDanger }}>🗑️ Delete</button>
                </div>
              </div>
            </div>
          );
  ...
  if (joinSuccess) {
    return (
      <div style={{ textAlign: 'center', padding: '3rem 1.5rem' }}>
        <div style={{ fontSize: 60, marginBottom: 16 }}>{joinSuccess.emoji}</div>
        <div style={{ fontFamily: "'Poppins',sans-serif", fontSize: 22, fontWeight: 700, marginBottom: 8 }}>You're in! 🎉</div>
        <div style={{ fontSize: 14, color: BRAND.textMuted, marginBottom: 24 }}>You've joined <strong style={{ color: BRAND.text }}>{joinSuccess.groupName}</strong> → {joinSuccess.destination}</div>
  ...
  return (
    <div>
      {confirmDelete && <ConfirmDialog title="Delete Trip" message={`Delete "${confirmDelete.groupName}"? All expenses, contacts and photos will be lost.`} confirmLabel="🗑️ Delete" confirmStyle="danger" onConfirm={() => { onDeleteTrip(confirmDelete.id); setConfirmDelete(null); setMenuOpen(null); }} onCancel={() => { setConfirmDelete(null); setMenuOpen(null); }} />}
      {confirmComplete && <ConfirmDialog title="Mark as Completed?" message={`"${confirmComplete.groupName}" will be moved to Past Trips. You can restore it anytime.`} confirmLabel="✅ Mark Complete" confirmStyle="primary" onConfirm={() => { onMarkComplete(confirmComplete.id); setConfirmComplete(null); setMenuOpen(null); }} onCancel={() => { setConfirmComplete(null); setMenuOpen(null); }} />}

      {/* Hero Banner */}
      <div style={{ position: 'relative', borderRadius: 20, padding: '1.75rem 1.5rem', marginBottom: '1.5rem', overflow: 'hidden', background: 'linear-gradient(135deg, #1a1040 0%, #0D1329 60%, #1a0d2e 100%)', border: `1px solid rgba(123,97,255,0.2)` }}>
        {/* Decorative gradient orbs */}
        <div style={{ position: 'absolute', top: -30, right: -20, width: 160, height: 160, borderRadius: '50%', background: 'radial-gradient(circle, rgba(123,97,255,0.15) 0%, transparent 70%)', pointerEvents: 'none' }} />
        <div style={{ position: 'absolute', bottom: -20, left: -10, width: 120, height: 120, borderRadius: '50%', background: 'radial-gradient(circle, rgba(255,106,174,0.1) 0%, transparent 70%)', pointerEvents: 'none' }} />
        <div style={{ position: 'absolute', top: -10, right: 40, fontSize: 80, opacity: 0.06, pointerEvents: 'none' }}>✈️</div>

        <div style={{ fontFamily: "'Poppins',sans-serif", fontSize: 11, fontWeight: 600, color: BRAND.purple, textTransform: 'uppercase', letterSpacing: 1.5, marginBottom: 8 }}>Your AI Travel Companion</div>
        <div style={{ fontFamily: "'Poppins',sans-serif", fontSize: 22, fontWeight: 700, color: BRAND.text, marginBottom: 6, lineHeight: 1.3 }}>Where to next? <span style={{ background: BRAND.gradient, WebkitBackgroundClip: 'text', WebkitTextFillColor: 'transparent', backgroundClip: 'text' }}>✈️</span></div>
        <div style={{ fontSize: 13, color: BRAND.textMuted, marginBottom: 20, lineHeight: 1.5 }}>Plan, split, explore — together.</div>
        <div style={{ display: 'flex', gap: 10, flexWrap: 'wrap' }}>
          <button style={{ ...S.btn, ...S.btnP, fontWeight: 600, fontSize: 13, padding: '10px 18px', borderRadius: 12 }} onClick={() => { setShowCreate(true); setShowJoin(false); }}>+ New Trip</button>
          <button style={{ ...S.btn, background: 'rgba(255,255,255,0.07)', color: BRAND.text, border: `1px solid rgba(255,255,255,0.12)`, fontSize: 13, padding: '10px 18px', borderRadius: 12 }} onClick={() => { setShowJoin(true); setShowCreate(false); }}>🔗 Join with Code</button>
        </div>
      </div>

      {/* Join form */}
      {showJoin && (
        <div style={{ ...S.card, border: `1px solid ${BRAND.border}`, background: 'rgba(123,97,255,0.05)', marginBottom: '1.25rem' }}>
          <div style={{ fontFamily: "'Poppins',sans-serif", fontSize: 15, fontWeight: 700, color: BRAND.purple, marginBottom: 12 }}>🔗 Join a Trip</div>
          <label style={S.label}>Share Code</label>
          <input style={{ ...S.input, letterSpacing: 2, fontFamily: "'Poppins',sans-serif", fontWeight: 600, textTransform: 'uppercase' }} value={joinCode} onChange={e => { setJoinCode(e.target.value); setJoinError(''); }} placeholder="e.g. JAI-4820" maxLength={10} />
          <label style={S.label}>Your Name</label>
          <input style={S.input} value={joinName} onChange={e => { setJoinName(e.target.value); setJoinError(''); }} placeholder="e.g. Rahul" />
          {joinError && <div style={{ fontSize: 12, color: BRAND.danger, marginTop: 8, padding: '7px 10px', background: BRAND.dangerBg, borderRadius: 8, border: `1px solid ${BRAND.dangerBorder}` }}>⚠️ {joinError}</div>}
          <div style={{ display: 'flex', gap: 8, marginTop: 12 }}>
            <button style={{ ...S.btn, ...S.btnP, flex: 1, justifyContent: 'center', padding: '10px', opacity: joining ? 0.6 : 1 }} onClick={handleJoin} disabled={!joinCode.trim() || !joinName.trim() || joining}>{joining ? 'Joining…' : '✓ Join Trip'}</button>
            <button style={S.btn} onClick={() => { setShowJoin(false); setJoinError(''); }}>✕</button>
          </div>
        </div>
      )}
      ...
      {/* Create form */}
      {showCreate && (
        <div style={{ ...S.card, border: `1px solid ${isSoloMode ? 'rgba(255,106,174,0.25)' : BRAND.border}`, background: isSoloMode ? 'rgba(255,106,174,0.04)' : 'rgba(123,97,255,0.04)', marginBottom: '1.25rem' }}>
          <div style={{ fontFamily: "'Poppins',sans-serif", fontSize: 15, fontWeight: 700, marginBottom: 14, background: isSoloMode ? 'linear-gradient(90deg,#FF6AAE,#FF8A3D)' : BRAND.gradient, WebkitBackgroundClip: 'text', WebkitTextFillColor: 'transparent', backgroundClip: 'text' }}>
            {isSoloMode ? '🎒 New Solo Adventure' : '✈️ Create New Group Trip'}
          </div>

          {/* Group/Solo toggle */}
          <div style={{ display: 'flex', gap: 0, background: BRAND.navyLight, borderRadius: 12, padding: 3, marginBottom: 16, border: `1px solid ${BRAND.borderLight}` }}>
            {[{ val: false, label: '👥 Group', desc: 'Travel with friends' }, { val: true, label: '🎒 Solo', desc: 'Just me, myself & I' }].map(opt => (
              <button key={String(opt.val)} onClick={() => { setIsSoloMode(opt.val); setForm(f => ({ ...f, emoji: opt.val ? '🎒' : '✈️', people: opt.val ? 1 : 2 })); }}
                style={{ flex: 1, padding: '9px 10px', borderRadius: 9, border: 'none', cursor: 'pointer', fontFamily: "'DM Sans',sans-serif",
                  background: isSoloMode === opt.val ? (opt.val ? 'linear-gradient(135deg,#FF6AAE,#FF8A3D)' : BRAND.gradientBtn) : 'transparent',
                  color: isSoloMode === opt.val ? '#fff' : BRAND.textMuted, fontWeight: 500, fontSize: 13, transition: 'all .2s',
                  boxShadow: isSoloMode === opt.val ? '0 2px 12px rgba(123,97,255,0.3)' : 'none' }}>
                {opt.label}
                <div style={{ fontSize: 10, opacity: 0.75, marginTop: 1 }}>{opt.desc}</div>
              </button>
            ))}
          ...
          <div style={{ display: 'flex', gap: 7, flexWrap: 'wrap', margin: '6px 0 10px' }}>
            {emojiOptions.map(e => (
              <div key={e} onClick={() => setForm(f => ({ ...f, emoji: e }))}
                style={{ width: 38, height: 38, borderRadius: 10, display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: 20, cursor: 'pointer',
                  border: form.emoji === e ? `2px solid ${isSoloMode ? BRAND.pink : BRAND.purple}` : `1px solid ${BRAND.borderLight}`,
                  background: form.emoji === e ? (isSoloMode ? 'rgba(255,106,174,0.12)' : 'rgba(123,97,255,0.12)') : BRAND.navyLight, transition: 'all .12s' }}>
                {e}
              </div>
            ))}
          </div>
          ...
          <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 10 }}>
            <div style={{ gridColumn: '1/-1' }}>
              <label style={S.label}>{isSoloMode ? 'Adventure Name *' : 'Group Name *'}</label>
              <input style={S.input} value={form.groupName} onChange={e => setForm(f => ({ ...f, groupName: e.target.value }))} placeholder={isSoloMode ? 'e.g. My Jaipur Chapter' : 'e.g. Pink City Explorers'} />
            </div>
            <div style={{ gridColumn: '1/-1' }}>
              <label style={S.label}>Destination *</label>
              <div onClick={() => { setShowDestPicker(true); setDestQuery(form.destination); setDestSuggestions([]); }}
                style={{ ...S.input, display: 'flex', alignItems: 'center', gap: 8, cursor: 'pointer', color: form.destination ? BRAND.text : BRAND.textDim, userSelect: 'none' }}>
                <span>📍</span>
                <span style={{ flex: 1 }}>{form.destination || 'Search city or place…'}</span>
                {form.destination && <span onClick={e => { e.stopPropagation(); setForm(f => ({ ...f, destination: '' })); }} style={{ fontSize: 13, color: BRAND.textDim, cursor: 'pointer' }}>✕</span>}
              </div>

              {showDestPicker && (
                <div style={{ position: 'fixed', inset: 0, zIndex: 500, background: BRAND.navy, display: 'flex', flexDirection: 'column' }}>
                  <div style={{ display: 'flex', alignItems: 'center', gap: 12, padding: '1rem 1.25rem', borderBottom: `1px solid ${BRAND.borderLight}`, background: BRAND.navyMid, flexShrink: 0 }}>
                    <button onClick={() => { setShowDestPicker(false); setDestSuggestions([]); }} style={{ width: 36, height: 36, borderRadius: '50%', border: `1px solid ${BRAND.borderLight}`, background: BRAND.navyLight, display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: 18, cursor: 'pointer', color: BRAND.text }}>←</button>
                    <div style={{ fontFamily: "'Poppins',sans-serif", fontSize: 17, fontWeight: 700, flex: 1 }}>Destination</div>
                  </div>
                  <div style={{ padding: '12px 14px', flexShrink: 0 }}>
                    <div style={{ display: 'flex', alignItems: 'center', gap: 8, background: BRAND.navyLight, borderRadius: 12, padding: '0 12px', border: `1px solid ${BRAND.border}` }}>
                      <span style={{ fontSize: 15 }}>🔍</span>
                      <input autoFocus style={{ ...S.input, border: 'none', background: 'transparent', flex: 1, padding: '10px 0', fontSize: 15, outline: 'none' }} value={destQuery} onChange={e => { setDestQuery(e.target.value); clearTimeout(destDebounce.current); destDebounce.current = setTimeout(() => searchDest(e.target.value), 350); }} placeholder="Search city or place…" />
                      {destLoading && <div style={{ width: 18, height: 18, border: `2px solid rgba(123,97,255,0.2)`, borderTopColor: BRAND.purple, borderRadius: '50%', animation: 'spin .75s linear infinite', flexShrink: 0 }} />}
                      {destQuery && !destLoading && <span onClick={() => { setDestQuery(''); setDestSuggestions([]); }} style={{ fontSize: 16, color: BRAND.textMuted, cursor: 'pointer' }}>✕</span>}
                    </div>
                  </div>
                  <div style={{ flex: 1, overflowY: 'auto' }}>
                    {destSuggestions.length > 0 && destSuggestions.map((item, i) => {
                      ...
                      const subText = [a.state, a.country].filter(Boolean).join(', ');
                      return (
                        <div key={item.osm_id + item.osm_type} onClick={() => { setForm(f => ({ ...f, destination: formatDestName(item) })); setShowDestPicker(false); setDestSuggestions([]); setDestQuery(''); }}
                          style={{ display: 'flex', alignItems: 'center', gap: 12, padding: '13px 16px', borderBottom: `1px solid ${BRAND.borderLight}`, cursor: 'pointer' }}>
                          <div style={{ width: 36, height: 36, borderRadius: 10, background: BRAND.navyLight, border: `1px solid ${BRAND.borderLight}`, display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: 17, flexShrink: 0 }}>{getDestIcon(item)}</div>
                          <div style={{ flex: 1, minWidth: 0 }}>
                            <div style={{ fontSize: 15, fontWeight: 600, color: BRAND.text, marginBottom: 2 }}>{mainText}</div>
                            {subText && <div style={{ fontSize: 12, color: BRAND.textMuted }}>{subText}</div>}
                          </div>
                          <span style={{ fontSize: 18, color: BRAND.textDim }}>›</span>
                        </div>
                      );
                    })}
                    {destQuery.length >= 2 && !destLoading && destSuggestions.length === 0 && (
                      <div style={{ textAlign: 'center', padding: '4rem 1.5rem', color: BRAND.textMuted }}>
                        <div style={{ fontSize: 40, marginBottom: 12 }}>🗺️</div>
                        <div style={{ fontSize: 15, fontWeight: 600, marginBottom: 6 }}>No results for "{destQuery}"</div>
                        <div style={{ fontSize: 13 }}>Try a different spelling or nearby city</div>
                      </div>
                    )}
                    {destQuery.length < 2 && <div style={{ textAlign: 'center', paddingTop: 40, color: BRAND.textDim, fontSize: 13 }}>Start typing to search destinations…</div>}
                  </div>
                  <div style={{ padding: '10px', textAlign: 'center', borderTop: `1px solid ${BRAND.borderLight}`, fontSize: 11, color: BRAND.textDim }}>© OpenStreetMap contributors</div>
                </div>
              )}
            </div>
            ...
            <div>
              <label style={S.label}>Date of Arrival *</label>
              <input style={S.input} type="date" value={form.arrival} min={today} max={maxDate} onChange={e => setForm(f => ({ ...f, arrival: e.target.value, departure: f.departure && f.departure < e.target.value ? '' : f.departure }))} />
              <div style={{ display: 'flex', gap: 5, marginTop: 7, flexWrap: 'wrap' }}>
                {[{ id: 'morning', label: '🌅 6AM–12PM' }, { id: 'afternoon', label: '☀️ 12–6PM' }, { id: 'evening', label: '🌆 6PM–12AM' }].map(slot => (
                  <button key={slot.id} type="button" onClick={() => setForm(f => ({ ...f, arrivalSlot: slot.id }))}
                    style={{ flex: 1, padding: '5px 4px', borderRadius: 8, border: `1.5px solid ${form.arrivalSlot === slot.id ? BRAND.purple : BRAND.borderLight}`, background: form.arrivalSlot === slot.id ? 'rgba(123,97,255,0.12)' : BRAND.navyLight, color: form.arrivalSlot === slot.id ? BRAND.purple : BRAND.textMuted, fontSize: 10, fontWeight: form.arrivalSlot === slot.id ? 600 : 400, cursor: 'pointer', fontFamily: "'DM Sans',sans-serif", transition: 'all .12s', whiteSpace: 'nowrap' }}>
                    {slot.label}
                  </button>
                ))}
              </div>
            ...
            <div>
              <label style={S.label}>Date of Departure *</label>
              <input style={S.input} type="date" value={form.departure} min={form.arrival || today} max={maxDate} onChange={e => setForm(f => ({ ...f, departure: e.target.value }))} />
              <div style={{ display: 'flex', gap: 5, marginTop: 7, flexWrap: 'wrap' }}>
                {[{ id: 'morning', label: '🌅 6AM–12PM' }, { id: 'afternoon', label: '☀️ 12–6PM' }, { id: 'evening', label: '🌆 6PM–12AM' }].map(slot => (
                  <button key={slot.id} type="button" onClick={() => setForm(f => ({ ...f, departureSlot: slot.id }))}
                    style={{ flex: 1, padding: '5px 4px', borderRadius: 8, border: `1.5px solid ${form.departureSlot === slot.id ? BRAND.purple : BRAND.borderLight}`, background: form.departureSlot === slot.id ? 'rgba(123,97,255,0.12)' : BRAND.navyLight, color: form.departureSlot === slot.id ? BRAND.purple : BRAND.textMuted, fontSize: 10, fontWeight: form.departureSlot === slot.id ? 600 : 400, cursor: 'pointer', fontFamily: "'DM Sans',sans-serif", transition: 'all .12s', whiteSpace: 'nowrap' }}>
                    {slot.label}
                  </button>
                ))}
              </div>
          ...
          <div style={{ display: 'flex', gap: 8, marginTop: 14 }}>
            <button style={{ ...S.btn, ...(isSoloMode ? S.btnSolo : S.btnP), flex: 1, justifyContent: 'center', padding: '11px', fontSize: 14, borderRadius: 12, opacity: creating ? 0.6 : 1 }} onClick={handleCreate} disabled={!form.groupName || !form.destination || !form.arrival || !form.departure || !form.createdBy || creating}>
              {creating ? 'Creating…' : isSoloMode ? '🎒 Start Solo Adventure' : '🚀 Create & Get Share Code'}
            </button>
            <button style={S.btn} onClick={() => setShowCreate(false)}>✕</button>
          </div>
      ...
      {activeTrips.length === 0 && !showCreate && !showJoin && (
        <div style={{ textAlign: 'center', padding: '3rem 1.5rem' }}>
          <div style={{ fontSize: 56, marginBottom: 14 }}>🗺️</div>
          <div style={{ fontFamily: "'Poppins',sans-serif", fontSize: 18, fontWeight: 700, marginBottom: 8 }}>No upcoming trips!</div>
          <div style={{ fontSize: 13, color: BRAND.textMuted, marginBottom: 24 }}>Create your first trip or join one with a code.</div>
          <button style={{ ...S.btn, ...S.btnP, padding: '10px 24px', fontSize: 14 }} onClick={() => setShowCreate(true)}>+ New Trip</button>
        </div>
      )}
      ...
      {activeTrips.map(trip => {
        ...
        return (
          <div key={trip.id} style={{ ...S.card, padding: 0, overflow: 'visible', marginBottom: 14, position: 'relative' }}>
            ...
            <div style={{ overflow: 'hidden', borderRadius: '14px 14px 0 0', cursor: 'pointer' }} onClick={() => onOpenTrip(trip.id)}>
              <div style={{ position: 'relative', height: 120, background: trip.coverUrl ? 'transparent' : (trip.isSolo ? 'linear-gradient(135deg,#2a1040,#1a0d2e,#0d1329)' : 'linear-gradient(135deg,#0d1a3a,#1a1040,#0d1329)'), overflow: 'hidden' }}>
                {/* Gradient orbs for visual interest */}
                {!trip.coverUrl && (
                  <>
                    <div style={{ position: 'absolute', top: -20, right: -20, width: 140, height: 140, borderRadius: '50%', background: `radial-gradient(circle, ${trip.isSolo ? 'rgba(255,106,174,0.2)' : 'rgba(123,97,255,0.2)'} 0%, transparent 70%)` }} />
                    <div style={{ position: 'absolute', bottom: -30, left: 20, width: 100, height: 100, borderRadius: '50%', background: `radial-gradient(circle, ${trip.isSolo ? 'rgba(255,138,61,0.15)' : 'rgba(0,212,160,0.12)'} 0%, transparent 70%)` }} />
                  </>
                )}
                {trip.coverUrl && <img src={trip.coverUrl} alt="" style={{ width: '100%', height: '100%', objectFit: 'cover' }} onError={e => e.target.style.display = 'none'} />}
                <div style={{ position: 'absolute', inset: 0, background: 'linear-gradient(180deg,rgba(0,0,0,0.05) 0%,rgba(0,0,0,0.55) 100%)' }} />
                <div style={{ position: 'absolute', top: 12, left: 14, fontSize: 28 }}>{trip.emoji}</div>
                <div style={{ position: 'absolute', top: 10, right: 12, display: 'flex', gap: 6 }}>
                  {trip.isSolo && <span style={{ fontSize: 10, fontWeight: 700, padding: '3px 10px', borderRadius: 10, background: 'rgba(255,106,174,0.25)', color: BRAND.pink, backdropFilter: 'blur(4px)', border: '1px solid rgba(255,106,174,0.3)' }}>Solo</span>}
                  <span style={{ fontSize: 11, fontWeight: 600, padding: '3px 10px', borderRadius: 10, background: status.bg, color: status.color, border: `1px solid ${status.border}` }}>{status.label}</span>
                </div>
                <div style={{ position: 'absolute', bottom: 12, left: 14, right: 14 }}>
                  <div style={{ fontFamily: "'Poppins',sans-serif", fontSize: 17, fontWeight: 700, color: '#fff', textShadow: '0 1px 8px rgba(0,0,0,0.5)' }}>{trip.groupName}</div>
            ...
            {/* Stats row */}
            <div style={{ padding: '12px 14px', display: 'flex', gap: 16, flexWrap: 'wrap', borderBottom: `1px solid ${BRAND.borderLight}`, cursor: 'pointer' }} onClick={() => onOpenTrip(trip.id)}>
              {[
                ['📅', formatDateRange(trip.arrival, trip.departure)],
                ['🌙', `${days} nights`],
                trip.isSolo ? ['💰', `₹${Math.round(totalSpend).toLocaleString('en-IN')} spent`] : ['👥', `${memberNames.length} members`],
                ...
                <div key={val} style={{ display: 'flex', alignItems: 'center', gap: 5, fontSize: 12, color: BRAND.textMuted }}>
                  <span>{icon}</span><span>{val}</span>
                </div>
              ))}
            </div>
            ...
            {/* Budget bar (solo) */}
            {trip.isSolo && trip.budget && (
              <div style={{ padding: '8px 14px', borderBottom: `1px solid ${BRAND.borderLight}`, cursor: 'pointer' }} onClick={() => onOpenTrip(trip.id)}>
                <div style={{ display: 'flex', justifyContent: 'space-between', fontSize: 11, color: BRAND.textMuted, marginBottom: 5 }}>
                  <span>Budget used</span>
                  <span style={{ fontWeight: 600, color: budgetPct > 85 ? BRAND.danger : BRAND.success }}>{budgetPct}% · ₹{Math.round(trip.budget - totalSpend).toLocaleString('en-IN')} left</span>
                </div>
                <div style={{ height: 4, background: BRAND.navyLight, borderRadius: 4, overflow: 'hidden' }}>
                  <div style={{ height: '100%', width: `${budgetPct}%`, borderRadius: 4, background: budgetPct > 85 ? BRAND.danger : BRAND.gradientBtn, transition: 'width .5s' }} />
                </div>
              </div>
            )}

            {/* Bottom row — members, share code, menu */}
            <div style={{ padding: '10px 14px', display: 'flex', alignItems: 'center', gap: 8, borderRadius: '0 0 14px 14px' }}>
              {trip.isSolo ? (
                <div style={{ display: 'flex', alignItems: 'center', gap: 8, cursor: 'pointer', flex: 1 }} onClick={() => onOpenTrip(trip.id)}>
                  <SoloAvatar initials={(memberNames[0] || 'ME').slice(0, 2)} size={28} />
                  <span style={{ fontSize: 12, color: BRAND.pink, fontWeight: 500 }}>Solo adventure by {memberNames[0] || 'You'}</span>
                </div>
              ) : (
                <div style={{ display: 'flex', cursor: 'pointer', flex: 1 }} onClick={() => onOpenTrip(trip.id)}>
                  {memberNames.slice(0, 5).map((m, i) => (
                    <div key={m + i} style={{ marginLeft: i > 0 ? -8 : 0, border: `2px solid ${BRAND.navyCard}`, borderRadius: '50%', zIndex: 5 - i }}>
                      <Avatar name={m} size={28} />
                    </div>
                  ))}
                  {memberNames.length > 5 && (
                    <div style={{ width: 28, height: 28, borderRadius: '50%', background: BRAND.navyLight, border: `2px solid ${BRAND.navyCard}`, display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: 10, color: BRAND.textMuted, marginLeft: -8 }}>
                      +{memberNames.length - 5}
                    </div>
                  )}
                </div>
              ...
              {/* Share code pill */}
              <div onClick={e => { e.stopPropagation(); copyCode(trip.shareCode, trip.id); }}
                style={{ display: 'flex', alignItems: 'center', gap: 6, background: 'rgba(123,97,255,0.12)', border: `1px solid ${BRAND.border}`, borderRadius: 20, padding: '5px 12px', cursor: 'pointer', flexShrink: 0 }}>
                <span style={{ fontFamily: "'Poppins',sans-serif", fontSize: 12, fontWeight: 700, color: BRAND.purple, letterSpacing: 1 }}>{trip.shareCode}</span>
                <span style={{ fontSize: 11, color: copied === trip.id ? BRAND.success : BRAND.purple }}>{copied === trip.id ? '✓' : '📋'}</span>
              </div>

              {/* Three-dot menu */}
              <div style={{ position: 'relative', flexShrink: 0 }}>
                <button onClick={e => { e.stopPropagation(); setMenuOpen(isMenuOpen ? null : trip.id); }} style={{ ...S.btn, padding: '6px 10px', fontSize: 16, color: BRAND.textMuted, lineHeight: 1 }}>⋯</button>
                {isMenuOpen && (
                  <>
                    <div style={{ position: 'fixed', inset: 0, zIndex: 100 }} onClick={() => setMenuOpen(null)} />
                    <div style={{ position: 'absolute', bottom: '110%', right: 0, background: BRAND.navyMid, border: `1px solid ${BRAND.borderLight}`, borderRadius: 12, boxShadow: '0 12px 40px rgba(0,0,0,0.5)', zIndex: 101, minWidth: 190, overflow: 'hidden' }}>
                      <button onClick={e => { e.stopPropagation(); setMenuOpen(null); setConfirmComplete(trip); }} style={{ display: 'flex', alignItems: 'center', gap: 10, width: '100%', padding: '12px 16px', border: 'none', background: 'none', fontSize: 13, cursor: 'pointer', color: BRAND.success, fontFamily: "'DM Sans',sans-serif", borderBottom: `1px solid ${BRAND.borderLight}` }}>✅ Mark as Completed</button>
                      <button onClick={e => { e.stopPropagation(); setMenuOpen(null); setConfirmDelete(trip); }} style={{ display: 'flex', alignItems: 'center', gap: 10, width: '100%', padding: '12px 16px', border: 'none', background: 'none', fontSize: 13, cursor: 'pointer', color: BRAND.danger, fontFamily: "'DM Sans',sans-serif" }}>🗑️ Delete Trip</button>
                    </div>
                  </>
                )}
              </div>
      ...
      {pastTrips.length > 0 && (
        <div onClick={() => setShowPast(true)} style={{ display: 'flex', alignItems: 'center', gap: 12, background: BRAND.navyCard, border: `1px solid ${BRAND.borderLight}`, borderRadius: 14, padding: '14px 18px', cursor: 'pointer', marginTop: 8 }}>
          <div style={{ width: 42, height: 42, borderRadius: 12, background: BRAND.navyLight, display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: 20, flexShrink: 0 }}>🗂️</div>
          <div style={{ flex: 1 }}>
            <div style={{ fontSize: 14, fontWeight: 600, marginBottom: 2 }}>Past Trips</div>
            <div style={{ fontSize: 12, color: BRAND.textMuted }}>{pastTrips.length} completed trip{pastTrips.length !== 1 ? 's' : ''} · tap to view memories</div>
          </div>
          <div style={{ fontSize: 16, color: BRAND.textDim }}>›</div>
        </div>
      )}
    </div>
  );
...
function ShareCodeModal({ trip, onDismiss }) {
  ...
  const isSolo = trip.isSolo;
  return (
    <div style={{ position: 'fixed', inset: 0, background: 'rgba(5,8,20,0.85)', backdropFilter: 'blur(12px)', zIndex: 500, display: 'flex', alignItems: 'center', justifyContent: 'center', padding: '1.5rem' }}>
      <div style={{ background: BRAND.navyMid, border: `1px solid ${BRAND.border}`, borderRadius: 24, padding: '2rem 1.75rem', maxWidth: 360, width: '100%', textAlign: 'center', boxShadow: '0 24px 80px rgba(0,0,0,0.6)' }}>
        <div style={{ fontSize: 56, marginBottom: 12 }}>{trip.emoji}</div>
        <div style={{ fontFamily: "'Poppins',sans-serif", fontSize: 20, fontWeight: 700, marginBottom: 6 }}>{isSolo ? 'Adventure Ready! 🎒' : 'Trip Created! 🎉'}</div>
        <div style={{ fontSize: 13, color: BRAND.textMuted, marginBottom: 24, lineHeight: 1.6 }}>
          {isSolo ? <>Your solo trip <strong style={{ color: BRAND.text }}>{trip.groupName}</strong> is set up.</> : <>Share this code with your friends to join <strong style={{ color: BRAND.text }}>{trip.groupName}</strong></>}
        </div>
        <div style={{ background: 'rgba(123,97,255,0.1)', border: `1px solid ${BRAND.border}`, borderRadius: 16, padding: '18px', marginBottom: 18 }}>
          <div style={{ fontSize: 11, color: BRAND.purple, fontWeight: 600, letterSpacing: .5, textTransform: 'uppercase', marginBottom: 8 }}>Your {isSolo ? 'Trip' : 'Share'} Code</div>
          <div style={{ fontFamily: "'Poppins',sans-serif", fontSize: 28, fontWeight: 700, background: BRAND.gradient, WebkitBackgroundClip: 'text', WebkitTextFillColor: 'transparent', backgroundClip: 'text', letterSpacing: 3 }}>{trip.shareCode}</div>
        </div>
        <button style={{ ...S.btn, ...S.btnP, width: '100%', justifyContent: 'center', padding: '12px', fontSize: 15, borderRadius: 12, marginBottom: 10, ...(copied ? { background: BRAND.successBg, color: BRAND.success, boxShadow: 'none', border: `1px solid ${BRAND.successBorder}` } : {}) }} onClick={copy}>
          {copied ? '✓ Copied!' : '📋 Copy Code'}
        </button>
        <button style={{ ...S.btn, width: '100%', justifyContent: 'center', padding: '10px', fontSize: 14, borderRadius: 12 }} onClick={onDismiss}>
          {isSolo ? 'Start Exploring →' : 'Open Trip →'}
...
function SoloExpensesPage({ trip, myNickname, onTripUpdate }) {
  ...
  const handleAdd = async () => {
    ...
    setSaving(true);
    try {
      const data = await addExpense(trip.id, { desc: form.desc, amount: parseFloat(form.amount), paidBy: myNickname || 'Me', cat: form.cat, split: [myNickname || 'Me'], note: form.note, date: form.date });
      setExpenses(es => [data.expense, ...es]);
      setForm({ desc: '', amount: '', cat: 'food', date: todayStr, note: '' });
      setShowForm(false);
    } catch (err) { alert('Could not save expense: ' + err.message); }
    setSaving(false);
  };
  ...
  return (
    <div>
      {/* Hero spending card */}
      <div style={{ background: 'linear-gradient(135deg, #1a0d2e, #0d1a3a, #0d1329)', borderRadius: 20, padding: '1.25rem 1.5rem', marginBottom: '1.25rem', position: 'relative', overflow: 'hidden', border: `1px solid rgba(255,106,174,0.15)` }}>
        <div style={{ position: 'absolute', top: -20, right: -20, width: 160, height: 160, borderRadius: '50%', background: 'radial-gradient(circle, rgba(255,106,174,0.15) 0%, transparent 70%)' }} />
        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', marginBottom: budget ? 12 : 0 }}>
          <div>
            <div style={{ fontSize: 11, color: BRAND.textMuted, fontWeight: 600, letterSpacing: .5, textTransform: 'uppercase', marginBottom: 4 }}>Total Spent</div>
            <div style={{ fontFamily: "'Poppins',sans-serif", fontSize: 30, fontWeight: 700, background: BRAND.gradient, WebkitBackgroundClip: 'text', WebkitTextFillColor: 'transparent', backgroundClip: 'text' }}>₹{Math.round(total).toLocaleString('en-IN')}</div>
            <div style={{ fontSize: 12, color: BRAND.textMuted, marginTop: 3 }}>₹{Math.round(dailyAvg).toLocaleString('en-IN')}/day avg · {expenses.length} entries</div>
          </div>
          {budget && (
            <div style={{ textAlign: 'right' }}>
              <div style={{ fontSize: 11, color: BRAND.textMuted, fontWeight: 600, letterSpacing: .3, textTransform: 'uppercase', marginBottom: 4 }}>Budget Left</div>
              <div style={{ fontFamily: "'Poppins',sans-serif", fontSize: 22, fontWeight: 700, color: budgetLeft < 0 ? BRAND.danger : BRAND.success }}>
                {budgetLeft < 0 ? '-' : ''}₹{Math.abs(Math.round(budgetLeft)).toLocaleString('en-IN')}
              </div>
              <div style={{ fontSize: 11, color: BRAND.textMuted, marginTop: 2 }}>of ₹{budget.toLocaleString('en-IN')}</div>
            </div>
          )}
        </div>
        {budget && (
          <div>
            <div style={{ height: 6, background: 'rgba(255,255,255,0.08)', borderRadius: 4, overflow: 'hidden' }}>
              <div style={{ height: '100%', borderRadius: 4, width: `${budgetPct}%`, background: budgetPct > 85 ? BRAND.danger : BRAND.gradientBtn, transition: 'width .6s' }} />
            </div>
            <div style={{ display: 'flex', justifyContent: 'space-between', marginTop: 5, fontSize: 11, color: BRAND.textDim }}>
              <span>{budgetPct}% used</span><span>{100 - budgetPct}% remaining</span>
            </div>
          </div>
        )}
        {!budget && <button onClick={() => setShowBudgetEdit(true)} style={{ ...S.btn, background: 'rgba(255,255,255,0.08)', color: BRAND.textMuted, border: `1px solid rgba(255,255,255,0.1)`, fontSize: 12, marginTop: 8 }}>+ Set a budget</button>}
        {budget && <button onClick={() => { setEditBudget(String(budget)); setShowBudgetEdit(true); }} style={{ ...S.btn, background: 'rgba(255,255,255,0.07)', color: BRAND.textMuted, border: `1px solid rgba(255,255,255,0.1)`, fontSize: 11, marginTop: 8 }}>✏️ Edit budget</button>}
      </div>

      {showBudgetEdit && (
        <div style={{ ...S.card, border: `1px solid ${BRAND.border}`, marginBottom: '1rem' }}>
          <label style={S.label}>Total trip budget ₹</label>
          <input style={S.input} type="number" value={editBudget} onChange={e => setEditBudget(e.target.value)} placeholder="e.g. 15000" autoFocus />
          <div style={{ display: 'flex', gap: 8, marginTop: 10 }}>
            <button style={{ ...S.btn, ...S.btnSolo, flex: 1, justifyContent: 'center', padding: '9px' }} onClick={() => { const v = parseFloat(editBudget); if (!isNaN(v) && v > 0) setBudget(v); setShowBudgetEdit(false); }}>✓ Save</button>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(3,1fr)', gap: 10, marginBottom: '1.25rem' }}>
        {[
          ...
          <div key={label} style={S.card}>
            <div style={{ fontSize: 18, marginBottom: 5 }}>{icon}</div>
            <div style={{ fontSize: 13, fontWeight: 600 }}>{label}</div>
            <div style={{ fontSize: 11, color: BRAND.textMuted, marginTop: 2 }}>{val}</div>
          </div>
        ))}
      </div>
      ...
      <div style={{ display: 'flex', gap: 6, flexWrap: 'wrap', marginBottom: '1.1rem', alignItems: 'center' }}>
        ...
        <button style={{ ...S.btn, marginLeft: 'auto', background: 'rgba(255,106,174,0.12)', color: BRAND.pink, border: `1px solid rgba(255,106,174,0.25)` }} onClick={() => setShowForm(v => !v)}>+ Add</button>
      </div>

      {showForm && (
        <div style={{ ...S.card, border: `1px solid rgba(255,106,174,0.25)`, marginBottom: '1rem' }}>
          <div style={{ fontFamily: "'Poppins',sans-serif", fontSize: 14, fontWeight: 700, color: BRAND.pink, marginBottom: 12 }}>Add expense</div>
          <div style={{ display: 'grid', gridTemplateColumns: '1fr 120px', gap: 10, marginBottom: 10 }}>
            <div>
              <label style={S.label}>What was it?</label>
              <input style={S.input} value={form.desc} onChange={e => setForm(f => ({ ...f, desc: e.target.value }))} placeholder="e.g. Amber Fort ticket" />
          ...
          <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 10, marginBottom: 10 }}>
            <div>
              <label style={S.label}>Category</label>
              <select style={{ ...S.input, appearance: 'none' }} value={form.cat} onChange={e => setForm(f => ({ ...f, cat: e.target.value }))}>
                {CATS.map(c => <option key={c.id} value={c.id}>{c.icon} {c.label}</option>)}
              </select>
            </div>
            <div>
          ...
          <input style={S.input} value={form.note} onChange={e => setForm(f => ({ ...f, note: e.target.value }))} placeholder="e.g. Amazing views!" />
          <div style={{ display: 'flex', gap: 8, marginTop: 12 }}>
            <button style={{ ...S.btn, ...S.btnSolo, flex: 1, justifyContent: 'center', padding: '10px', opacity: saving ? 0.6 : 1 }} onClick={handleAdd} disabled={!form.desc || !form.amount || saving}>{saving ? 'Saving…' : '✓ Add expense'}</button>
            <button style={S.btn} onClick={() => setShowForm(false)}>✕</button>
          </div>
        </div>
      )}
      ...
      {section === 'log' && (
        <div>
          <div style={{ display: 'flex', gap: 5, flexWrap: 'wrap', marginBottom: '1rem' }}>
            ...
            {CATS.filter(c => catTotals[c.id] > 0).map(c => (
              <button key={c.id} onClick={() => setFilterCat(filterCat === c.id ? 'all' : c.id)}
                style={{ ...S.btn, fontSize: 11, padding: '4px 10px', borderRadius: 16, background: filterCat === c.id ? c.bg : BRAND.navyLight, color: filterCat === c.id ? c.color : BRAND.textMuted, border: `1px solid ${filterCat === c.id ? c.color + '44' : BRAND.borderLight}` }}>
                {c.icon} {c.label}
              </button>
            ))}
          </div>
          {sortedFiltered.length === 0 && <div style={{ textAlign: 'center', padding: '2.5rem', color: BRAND.textMuted, fontSize: 14 }}><div style={{ fontSize: 40, marginBottom: 10 }}>📝</div><p>No expenses yet. Add your first one!</p></div>}
          {sortedFiltered.map(exp => {
            const cat = CATS.find(c => c.id === exp.cat) || CATS[4];
            return (
              <div key={exp.id} style={{ ...S.card, display: 'flex', alignItems: 'center', gap: 12, marginBottom: 8, borderLeft: `3px solid ${cat.color}` }}>
                <div style={{ width: 40, height: 40, borderRadius: 11, display: 'flex', alignItems: 'center', justifyContent: 'center', background: cat.bg, flexShrink: 0, fontSize: 18 }}>{cat.icon}</div>
                <div style={{ flex: 1, minWidth: 0 }}>
                  <div style={{ fontSize: 14, fontWeight: 500 }}>{exp.desc}</div>
                ...
                <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'flex-end', gap: 4 }}>
                  <div style={{ fontFamily: "'Poppins',sans-serif", fontSize: 15, fontWeight: 700, background: BRAND.gradient, WebkitBackgroundClip: 'text', WebkitTextFillColor: 'transparent', backgroundClip: 'text' }}>₹{exp.amount.toLocaleString('en-IN')}</div>
                  <button onClick={() => handleDelete(exp.id)} style={{ ...S.btn, padding: '2px 7px', fontSize: 11, color: BRAND.textDim, borderColor: 'transparent', background: 'transparent' }}>✕</button>
                </div>
              </div>
            );
          })}
              ...
              <div key={c.id} style={{ ...S.card, marginBottom: 8 }}>
                <div style={{ display: 'flex', alignItems: 'center', gap: 10, marginBottom: 8 }}>
                  ...
                  <div style={{ flex: 1 }}>
                    <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                      <span style={{ fontSize: 13, fontWeight: 600 }}>{c.label}</span>
                      <span style={{ fontFamily: "'Poppins',sans-serif", fontSize: 14, fontWeight: 700, color: c.color }}>₹{Math.round(catTotals[c.id]).toLocaleString('en-IN')}</span>
                    </div>
                    <div style={{ display: 'flex', alignItems: 'center', gap: 8, marginTop: 5 }}>
                      <div style={{ flex: 1, height: 5, background: BRAND.navyLight, borderRadius: 4, overflow: 'hidden' }}>
                        <div style={{ height: '100%', width: `${pct}%`, borderRadius: 4, background: c.color, transition: 'width .5s' }} />
                      </div>
                      <span style={{ fontSize: 11, color: BRAND.textMuted, width: 32, textAlign: 'right' }}>{pct}%</span>
                    </div>
                  </div>
                </div>
              </div>
...
function ContactsPage({ trip, myNickname, isSolo }) {
  ...
  const handleAdd = async () => {
    ...
    try {
      if (form._editId) {
        setContacts(cs => cs.map(c => c.id === form._editId ? { ...c, name: form.name, role: form.role, cat: form.cat, phone: form.phone, note: form.note } : c));
      } else {
        const data = await addContact(trip.id, { ...form, addedBy: myNickname || 'Me' });
        setContacts(cs => [...cs, data.contact]);
      }
      setForm({ name: '', role: '', cat: 'driver', phone: '', note: '' });
      setShowForm(false);
    } catch (err) { alert('Could not save contact: ' + err.message); }
    setSaving(false);
  };

  const handleDelete = async (cid) => {
    try { await deleteContact(trip.id, cid); setContacts(cs => cs.filter(x => x.id !== cid)); }
    catch (err) { alert('Could not delete: ' + err.message); }
  };
  ...
  if (showForm) return (
    <div style={{ position: 'fixed', inset: 0, background: BRAND.navy, zIndex: 400, display: 'flex', flexDirection: 'column' }}>
      <div style={{ display: 'flex', alignItems: 'center', gap: 12, padding: '1rem 1.25rem', background: BRAND.navyMid, borderBottom: `1px solid ${BRAND.borderLight}`, flexShrink: 0 }}>
        <button onClick={() => { setShowForm(false); setForm({ name: '', role: '', cat: 'driver', phone: '', note: '' }); }} style={{ width: 36, height: 36, borderRadius: '50%', border: `1px solid ${BRAND.borderLight}`, background: BRAND.navyLight, display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: 18, cursor: 'pointer', color: BRAND.text }}>←</button>
        <div style={{ fontFamily: "'Poppins',sans-serif", fontSize: 17, fontWeight: 700, flex: 1 }}>{form._editId ? 'Edit Contact' : 'Add Contact'}</div>
        <button onClick={handleAdd} disabled={saving || !form.name.trim() || !form.phone.trim()} style={{ ...S.btn, ...S.btnP, padding: '8px 22px', fontSize: 14, fontWeight: 600, borderRadius: 12, opacity: (saving || !form.name.trim() || !form.phone.trim()) ? 0.4 : 1 }}>{saving ? 'Saving…' : form._editId ? 'Update' : 'Save'}</button>
      </div>
      <div style={{ flex: 1, overflowY: 'auto' }}>
        <div style={{ background: 'linear-gradient(135deg,#1a0d2e,#0d1329)', padding: '1.5rem 1.25rem 2rem' }}>
          <div style={{ fontSize: 11, color: BRAND.textMuted, fontWeight: 600, letterSpacing: .5, textTransform: 'uppercase', marginBottom: 12 }}>Category</div>
          <div style={{ display: 'flex', gap: 8, flexWrap: 'wrap' }}>
            {CONTACT_CATS.map(c => (
              <button key={c.id} onClick={() => setForm(f => ({ ...f, cat: c.id }))}
                style={{ display: 'flex', alignItems: 'center', gap: 6, padding: '7px 13px', borderRadius: 20, fontSize: 13, border: `1.5px solid ${form.cat === c.id ? BRAND.purple : 'rgba(255,255,255,0.12)'}`, background: form.cat === c.id ? 'rgba(123,97,255,0.2)' : 'rgba(255,255,255,0.05)', color: form.cat === c.id ? BRAND.purple : BRAND.textMuted, cursor: 'pointer', fontFamily: "'DM Sans',sans-serif", fontWeight: form.cat === c.id ? 700 : 400 }}>
                {c.icon} {c.label}
              </button>
            ))}
          </div>
        </div>
        <div style={{ background: BRAND.navyMid, borderRadius: '20px 20px 0 0', marginTop: -16, padding: '1.5rem 1.25rem 3rem' }}>
          <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 12, marginBottom: '1.25rem' }}>
            <div><label style={S.label}>Full Name *</label><input style={{ ...S.input, marginTop: 6 }} value={form.name} onChange={e => setForm(f => ({ ...f, name: e.target.value }))} placeholder="e.g. Ramesh Kumar" autoFocus /></div>
            <div><label style={S.label}>Role</label><input style={{ ...S.input, marginTop: 6 }} value={form.role} onChange={e => setForm(f => ({ ...f, role: e.target.value }))} placeholder="e.g. Hotel Manager" /></div>
          </div>
          <div style={{ marginBottom: '1.25rem' }}>
            <label style={S.label}>Phone *</label>
            <input style={{ ...S.input, fontSize: 16, padding: '12px 14px', marginTop: 6, letterSpacing: .5 }} value={form.phone} onChange={e => setForm(f => ({ ...f, phone: e.target.value }))} placeholder="+91 98765 43210" type="tel" />
          </div>
          <div style={{ marginBottom: '1.25rem' }}>
            <label style={S.label}>Note <span style={{ color: BRAND.textDim, fontWeight: 400, textTransform: 'none' }}>(optional)</span></label>
            <textarea style={{ ...S.input, resize: 'none', minHeight: 72, marginTop: 6, lineHeight: 1.5 }} value={form.note} onChange={e => setForm(f => ({ ...f, note: e.target.value }))} placeholder="Any useful info…" />
          </div>
        </div>
      </div>
    </div>
  ...
  return (
    <div style={{ paddingBottom: '5rem' }}>
      <div style={{ background: 'linear-gradient(135deg, rgba(123,97,255,0.08), rgba(255,106,174,0.05))', border: `1px solid ${BRAND.border}`, borderRadius: 16, padding: '1rem 1.25rem', marginBottom: '1.1rem', display: 'flex', alignItems: 'center', gap: 14 }}>
        <div style={{ fontSize: 34 }}>📒</div>
        <div style={{ flex: 1 }}>
          <div style={{ fontFamily: "'Poppins',sans-serif", fontSize: 15, fontWeight: 700, marginBottom: 2 }}>{isSolo ? 'My Contacts' : 'Trip Contacts'}</div>
          <div style={{ fontSize: 12, color: BRAND.textMuted, lineHeight: 1.5 }}>{isSolo ? 'Personal contacts for this trip.' : 'Shared by the group — drivers, hotel, guides & emergency.'}</div>
        </div>
        <div style={{ textAlign: 'right' }}>
          <div style={{ fontFamily: "'Poppins',sans-serif", fontSize: 24, fontWeight: 700, background: BRAND.gradient, WebkitBackgroundClip: 'text', WebkitTextFillColor: 'transparent', backgroundClip: 'text' }}>{contacts.length}</div>
          <div style={{ fontSize: 11, color: BRAND.textMuted }}>saved</div>
        </div>
      </div>

      <div style={{ marginBottom: '0.75rem' }}>
        <input style={{ ...S.input }} placeholder="🔍  Search by name or role…" value={search} onChange={e => setSearch(e.target.value)} />
      </div>

      <div style={{ display: 'flex', gap: 5, flexWrap: 'wrap', marginBottom: '1rem' }}>
        <button onClick={() => setFilterCat('all')} style={{ ...S.btn, fontSize: 11, padding: '4px 12px', borderRadius: 20, ...(filterCat === 'all' ? S.btnP : {}) }}>All · {contacts.length}</button>
        {CONTACT_CATS.filter(c => catCounts[c.id] > 0).map(c => (
          <button key={c.id} onClick={() => setFilterCat(filterCat === c.id ? 'all' : c.id)}
            style={{ ...S.btn, fontSize: 11, padding: '4px 12px', borderRadius: 20, background: filterCat === c.id ? c.bg : BRAND.navyLight, color: filterCat === c.id ? c.color : BRAND.textMuted, border: `1px solid ${filterCat === c.id ? c.color + '44' : BRAND.borderLight}` }}>
            {c.icon} {c.label} · {catCounts[c.id]}
          </button>
        ))}
      </div>
      ...
      {filtered.length === 0 && (
        <div style={{ textAlign: 'center', padding: '4rem 1rem', color: BRAND.textMuted }}>
          <div style={{ fontSize: 48, marginBottom: 14 }}>📭</div>
          <p style={{ fontSize: 15, fontWeight: 500, marginBottom: 6 }}>No contacts yet</p>
          <p style={{ fontSize: 13 }}>Tap + to add your first one</p>
        </div>
      ...
      {filtered.map(c => {
        const cm = getCat(c.cat);
        return (
          <div key={c.id} style={{ background: BRAND.navyCard, border: `1px solid ${BRAND.borderLight}`, borderRadius: 16, marginBottom: 10, overflow: 'hidden' }}>
            ...
            <div style={{ padding: '14px 16px' }}>
              <div style={{ display: 'flex', alignItems: 'center', gap: 10, marginBottom: 12 }}>
                ...
                <div style={{ flex: 1, minWidth: 0 }}>
                  <div style={{ display: 'flex', alignItems: 'center', gap: 7, flexWrap: 'wrap' }}>
                    <span style={{ fontFamily: "'Poppins',sans-serif", fontSize: 15, fontWeight: 700 }}>{c.name}</span>
                    <span style={{ fontSize: 10, fontWeight: 700, padding: '2px 8px', borderRadius: 10, background: cm.bg, color: cm.color }}>{cm.label}</span>
                  </div>
                </div>
                <button onClick={() => { setForm({ name: c.name, role: c.role || '', cat: c.cat, phone: c.phone, note: c.note || '', _editId: c.id }); setShowForm(true); }} style={{ width: 28, height: 28, borderRadius: '50%', border: `1px solid ${BRAND.borderLight}`, background: BRAND.navyLight, display: 'flex', alignItems: 'center', justifyContent: 'center', cursor: 'pointer', fontSize: 12 }}>✏️</button>
                <button onClick={() => handleDelete(c.id)} style={{ width: 28, height: 28, borderRadius: '50%', border: `1px solid ${BRAND.borderLight}`, background: BRAND.navyLight, display: 'flex', alignItems: 'center', justifyContent: 'center', cursor: 'pointer', fontSize: 12, color: BRAND.textMuted }}>✕</button>
              </div>
              <div style={{ display: 'flex', flexDirection: 'column', gap: 7, paddingLeft: 2 }}>
                {c.role && (
                  <div style={{ display: 'flex', alignItems: 'center', gap: 10 }}>
                    <span style={{ fontSize: 11, fontWeight: 600, color: BRAND.textDim, textTransform: 'uppercase', letterSpacing: .4, width: 52, flexShrink: 0 }}>Role</span>
                    <span style={{ fontSize: 13, color: BRAND.text, fontWeight: 500 }}>{c.role}</span>
                  </div>
                )}
                <div style={{ display: 'flex', alignItems: 'center', gap: 10 }}>
                  <span style={{ fontSize: 11, fontWeight: 600, color: BRAND.textDim, textTransform: 'uppercase', letterSpacing: .4, width: 52, flexShrink: 0 }}>Call</span>
                  <a href={`tel:${c.phone}`} style={{ display: 'inline-flex', alignItems: 'center', gap: 6, background: cm.bg, border: `1px solid ${cm.color}33`, borderRadius: 8, padding: '5px 12px', fontSize: 13, fontWeight: 600, color: cm.color, textDecoration: 'none' }}>📞 {c.phone}</a>
                </div>
                {c.note && (
                  <div style={{ display: 'flex', alignItems: 'flex-start', gap: 10 }}>
                    <span style={{ fontSize: 11, fontWeight: 600, color: BRAND.textDim, textTransform: 'uppercase', letterSpacing: .4, width: 52, flexShrink: 0, paddingTop: 1 }}>Note</span>
                    <span style={{ fontSize: 13, color: BRAND.textMuted, lineHeight: 1.55 }}>{c.note}</span>
                  </div>
                )}
              </div>
            </div>
      ...
      {contacts.filter(c => c.cat === 'emergency' || c.cat === 'medical').length > 0 && (
        <div style={{ background: 'rgba(255,183,3,0.06)', border: `1px solid rgba(255,183,3,0.2)`, borderRadius: 14, padding: '1rem 1.25rem', marginTop: '0.5rem' }}>
          <div style={{ fontSize: 11, fontWeight: 700, color: BRAND.yellow, marginBottom: 10, textTransform: 'uppercase', letterSpacing: .4 }}>🚨 Quick-dial emergency</div>
          <div style={{ display: 'flex', gap: 8, flexWrap: 'wrap' }}>
            {contacts.filter(c => c.cat === 'emergency' || c.cat === 'medical').map(c => (
              <a key={c.id} href={`tel:${c.phone}`} style={{ display: 'inline-flex', alignItems: 'center', gap: 6, background: BRAND.navyLight, border: `1px solid rgba(255,183,3,0.25)`, borderRadius: 10, padding: '8px 14px', fontSize: 13, fontWeight: 600, color: BRAND.text, textDecoration: 'none' }}>📞 {c.name}</a>
            ))}
          </div>
        </div>
      )}
      ...
      <button onClick={() => setShowForm(true)}
        style={{ position: 'fixed', bottom: 24, right: 20, width: 58, height: 58, borderRadius: '50%', background: BRAND.gradientBtn, border: 'none', boxShadow: '0 4px 20px rgba(123,97,255,0.45)', cursor: 'pointer', display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: 28, color: '#fff', zIndex: 300 }}>
        +
      </button>
    </div>
  );
...
function LocalTastePage({ destination, isSolo, autoData, autoStep, onRetry }) {
  ...
  const toggleDone = (key) => setDoneItems(prev => { const n = new Set(prev); n.has(key) ? n.delete(key) : n.add(key); return n; });
  const tagBg = t => {
    if (['must-try','must-do','iconic'].includes(t)) return { bg: 'rgba(255,77,106,0.12)', color: BRAND.danger };
    if (['heritage','scenic','culture','offbeat'].includes(t)) return { bg: 'rgba(55,138,221,0.12)', color: '#378ADD' };
    return { bg: 'rgba(255,183,3,0.12)', color: BRAND.yellow };
  };
  ...
  const Sec = ({ icon, title, items, iconBg, secKey }) => {
    ...
    return (
      <div style={{ marginBottom: '1.25rem' }}>
        <div style={{ display: 'flex', alignItems: 'center', gap: 8, marginBottom: 10 }}>
          <div style={{ width: 32, height: 32, borderRadius: 9, background: iconBg, display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: 16 }}>{icon}</div>
          <span style={{ fontSize: 12, fontWeight: 700, letterSpacing: .4, textTransform: 'uppercase', color: BRAND.textMuted }}>{title}</span>
          <span style={{ fontSize: 11, color: BRAND.textDim }}>{items.length} picks</span>
          {doneCount > 0 && <span style={{ fontSize: 11, fontWeight: 600, color: BRAND.success, background: BRAND.successBg, border: `1px solid ${BRAND.successBorder}`, borderRadius: 10, padding: '2px 8px', marginLeft: 'auto' }}>✓ {doneCount}/{items.length} done</span>}
        </div>
        {items.map((item, i) => {
          const key = `${secKey}-${i}`;
          const isDone = doneItems.has(key);
          return (
            <div key={i} style={{ ...S.card, display: 'flex', gap: 14, alignItems: 'flex-start', opacity: isDone ? 0.45 : 1, transition: 'all .25s' }}>
              <div style={{ fontSize: 28, lineHeight: 1, flexShrink: 0, filter: isDone ? 'grayscale(1)' : 'none' }}>{item.emoji}</div>
              <div style={{ flex: 1, minWidth: 0 }}>
                <div style={{ fontSize: 14, fontWeight: 600, marginBottom: 3, textDecoration: isDone ? 'line-through' : 'none', color: isDone ? BRAND.textDim : BRAND.text }}>{item.name}</div>
                <div style={{ fontSize: 12, color: BRAND.textMuted, lineHeight: 1.5 }}>{item.desc}</div>
                <div style={{ display: 'flex', gap: 5, flexWrap: 'wrap', marginTop: 7 }}>
                  {(item.tags || []).map(t => { const c = tagBg(t); return <span key={t} style={{ fontSize: 10, fontWeight: 600, padding: '2px 8px', borderRadius: 10, textTransform: 'uppercase', letterSpacing: .3, background: isDone ? BRAND.navyLight : c.bg, color: isDone ? BRAND.textDim : c.color }}>{t}</span>; })}
                </div>
              </div>
              <button onClick={() => toggleDone(key)} style={{ flexShrink: 0, width: 32, height: 32, borderRadius: '50%', border: isDone ? `2px solid ${BRAND.success}` : `1.5px solid ${BRAND.borderLight}`, background: isDone ? BRAND.success : BRAND.navyLight, color: isDone ? '#fff' : BRAND.textDim, fontSize: 16, fontWeight: 700, cursor: 'pointer', display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
                {isDone ? '✓' : '○'}
              </button>
            </div>
          );
  ...
  if (step === 'loading') return <Spinner text={`Discovering local flavours of ${dest}…`} />;

  if (step === 'result' && data) return (
    <div>
      <div style={{ background: 'rgba(255,183,3,0.06)', border: `1px solid rgba(255,183,3,0.2)`, borderRadius: 14, padding: '1.1rem 1.25rem', marginBottom: '1.25rem', display: 'flex', gap: 14, alignItems: 'center' }}>
        <div style={{ fontSize: 36 }}>🗺️</div>
        <div style={{ flex: 1 }}>
          <div style={{ fontFamily: "'Poppins',sans-serif", fontSize: 17, fontWeight: 700, color: BRAND.yellow, marginBottom: 3 }}>{data.headline}</div>
          <div style={{ fontSize: 12, color: BRAND.textMuted, lineHeight: 1.5 }}>{data.tagline}</div>
        </div>
        <button style={{ ...S.btn, fontSize: 12 }} onClick={() => { setStep('idle'); setData(null); setDoneItems(new Set()); }}>↺</button>
      </div>
      <Sec icon="🍴" iconBg="rgba(255,183,3,0.12)" title="Must-eat dishes" items={data.dishes || []} secKey="dishes" />
      <Sec icon="📍" iconBg="rgba(55,138,221,0.12)" title="Unmissable places" items={data.places || []} secKey="places" />
      <Sec icon="✨" iconBg="rgba(123,97,255,0.12)" title="Local experiences" items={data.experiences || []} secKey="exp" />
      {data.tip && <div style={{ background: BRAND.navyLight, border: `1px solid ${BRAND.border}`, borderRadius: 10, padding: '.75rem 1rem', display: 'flex', gap: 10, alignItems: 'flex-start', marginTop: '1rem', fontSize: 12, color: BRAND.textMuted, lineHeight: 1.5 }}>💡 <span><strong style={{ color: BRAND.text }}>Local tip:</strong> {data.tip}</span></div>}
    </div>
  );

  return (
    <div>
      <div style={{ background: 'linear-gradient(135deg,rgba(255,183,3,0.08),rgba(255,138,61,0.06))', border: `1px solid rgba(255,183,3,0.15)`, borderRadius: 14, padding: '1.5rem', marginBottom: '1.25rem', textAlign: 'center' }}>
        <div style={{ fontSize: 40, marginBottom: 10 }}>🍽️</div>
        <div style={{ fontFamily: "'Poppins',sans-serif", fontSize: 18, fontWeight: 700, marginBottom: 6 }}>Local Taste Guide</div>
        <div style={{ fontSize: 13, color: BRAND.textMuted, lineHeight: 1.6 }}>Discover what to eat, where to go, and what to do like a local.</div>
      </div>
      <div style={S.card}>
        <label style={S.label}>Destination</label>
        <input style={S.input} value={dest} onChange={e => setDest(e.target.value)} onKeyDown={e => e.key === 'Enter' && generate()} placeholder="e.g. Jaipur, Rajasthan" />
        <button style={{ ...S.btn, ...S.btnP, width: '100%', justifyContent: 'center', marginTop: 12, padding: '11px', fontSize: 14, borderRadius: 12 }} onClick={generate} disabled={!dest.trim()}>✨ Discover local flavours</button>
      </div>
      <div style={{ display: 'flex', gap: 8, flexWrap: 'wrap', marginTop: 8 }}>
        {['Jaipur','Udaipur','Goa','Varanasi','Mumbai','Coorg','Hampi'].map(c => (
          <button key={c} style={{ ...S.btn, fontSize: 12, padding: '5px 12px', borderRadius: 20 }} onClick={() => setDest(c)}>{c}</button>
...
function SplitPage({ trip, myNickname }) {
  ...
  const [budgetInput, setBudgetInput] = useState('');
  const todayStr = new Date().toISOString().split('T')[0];
  const [form, setForm] = useState({ desc: '', amount: '', paidBy: myNickname || memberNames[0] || '', cat: 'food', date: todayStr, splitMode: 'all', splitWith: [...memberNames], _splitOpen: false });
  ...
  function renderCharts() {
    ...
    if (donutRef.current && budget) {
      chartInstances.current.donut = new window.Chart(donutRef.current, {
        type: 'doughnut',
        data: { datasets: [{ data: [Math.min(total, budget), Math.max(0, budget - total)], backgroundColor: [budgetPct > 85 ? BRAND.danger : BRAND.purple, 'rgba(123,97,255,0.1)'], borderWidth: 0 }] },
        options: { responsive: true, maintainAspectRatio: false, cutout: '74%', plugins: { legend: { display: false }, tooltip: { callbacks: { label: ctx => ctx.dataIndex === 0 ? ` Spent: ₹${Math.round(Math.min(total, budget)).toLocaleString('en-IN')}` : ` Left: ₹${Math.round(Math.max(0, budget - total)).toLocaleString('en-IN')}` } } } },
        plugins: [{ id: 'center', afterDraw(chart) { const { ctx, chartArea: { width, height, left, top } } = chart; const cx = left + width / 2, cy = top + height / 2; ctx.save(); ctx.textAlign = 'center'; ctx.textBaseline = 'middle'; ctx.font = '600 17px system-ui'; ctx.fillStyle = BRAND.text; ctx.fillText(`${budgetPct}%`, cx, cy - 9); ctx.font = '12px system-ui'; ctx.fillStyle = BRAND.textMuted; ctx.fillText('used', cx, cy + 9); ctx.restore(); } }]
      });
    }
    if (barRef.current) {
      const activeCats = CATS.filter(c => catTotals[c.id] > 0);
      ...
      chartInstances.current.bar = new window.Chart(barRef.current, {
        type: 'bar',
        data: { labels: activeCats.map(c => c.label), datasets: [{ data: activeCats.map(c => catTotals[c.id]), backgroundColor: activeCats.map(c => c.color), borderRadius: 6, borderSkipped: false }] },
        options: { responsive: true, maintainAspectRatio: false, plugins: { legend: { display: false }, tooltip: { callbacks: { label: ctx => ` ₹${Math.round(ctx.raw).toLocaleString('en-IN')}` } } }, scales: { x: { ticks: { color: BRAND.textMuted, font: { size: 11 } }, grid: { display: false }, border: { display: false } }, y: { ticks: { color: BRAND.textMuted, font: { size: 11 }, callback: v => `₹${v >= 1000 ? Math.round(v / 1000) + 'k' : v}` }, grid: { color: BRAND.borderLight }, border: { display: false } } } }
      });
    }
  }
  ...
  const handleAdd = async () => {
    ...
    setSaving(true);
    try {
      const data = await addExpense(trip.id, { desc: form.desc, amount: parseFloat(form.amount), paidBy: form.paidBy, cat: form.cat, split: splitWith, date: form.date });
      setExpenses(es => [data.expense, ...es]);
      setForm({ desc: '', amount: '', paidBy: myNickname || memberNames[0] || '', cat: 'food', date: todayStr, splitMode: 'all', splitWith: [...memberNames], _splitOpen: false });
      setShowForm(false);
    } catch (err) { alert('Could not save: ' + err.message); }
  ...
  const SECTION_TABS = [{ id: 'expenses', label: 'Expenses' }, { id: 'shares', label: 'Shares' }, { id: 'balances', label: 'Balances' }, { id: 'insights', label: 'Insights' }];

  /* Fullscreen expense form */
  if (showForm) return (
    <div style={{ position: 'fixed', inset: 0, background: BRAND.navy, zIndex: 400, display: 'flex', flexDirection: 'column', animation: 'slideUp .25s ease-out' }}>
      <style>{`@keyframes slideUp{from{opacity:0;transform:translateY(40px)}to{opacity:1;transform:translateY(0)}}`}</style>
      <div style={{ display: 'flex', alignItems: 'center', gap: 12, padding: '1rem 1.25rem', background: BRAND.navyMid, borderBottom: `1px solid ${BRAND.borderLight}`, flexShrink: 0 }}>
        <button onClick={() => setShowForm(false)} style={{ width: 36, height: 36, borderRadius: '50%', border: `1px solid ${BRAND.borderLight}`, background: BRAND.navyLight, display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: 18, cursor: 'pointer', color: BRAND.text }}>←</button>
        <div style={{ fontFamily: "'Poppins',sans-serif", fontSize: 17, fontWeight: 700, flex: 1 }}>Add Expense</div>
        <button onClick={handleAdd} disabled={saving || !form.desc || !form.amount} style={{ ...S.btn, ...S.btnP, padding: '8px 22px', fontSize: 14, fontWeight: 600, borderRadius: 12, opacity: (saving || !form.desc || !form.amount) ? 0.4 : 1 }}>{saving ? 'Saving…' : 'Save'}</button>
      </div>
      <div style={{ flex: 1, overflowY: 'auto' }}>
        <div style={{ background: 'linear-gradient(135deg,#0d1a3a,#1a1040)', padding: '2rem 1.5rem 2.5rem', textAlign: 'center' }}>
          <div style={{ fontSize: 11, color: BRAND.textMuted, fontWeight: 600, letterSpacing: .6, textTransform: 'uppercase', marginBottom: 12 }}>How much?</div>
          <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'center', gap: 6 }}>
            <span style={{ fontFamily: "'Poppins',sans-serif", fontSize: 28, fontWeight: 700, color: BRAND.textDim }}>₹</span>
            <input type="number" placeholder="0" value={form.amount} onChange={e => setForm(f => ({ ...f, amount: e.target.value }))} autoFocus
              style={{ fontFamily: "'Poppins',sans-serif", fontSize: 56, fontWeight: 700, background: BRAND.gradient, WebkitBackgroundClip: 'text', WebkitTextFillColor: 'transparent', backgroundClip: 'text', border: 'none', background: 'transparent', outline: 'none', width: '65%', textAlign: 'center', padding: 0, color: BRAND.text }} />
          </div>
        </div>
        <div style={{ background: BRAND.navyMid, borderRadius: '20px 20px 0 0', marginTop: -16, padding: '1.5rem 1.25rem 2rem', minHeight: '100%' }}>
          <div style={{ marginBottom: '1.25rem' }}>
            <label style={S.label}>What was it?</label>
            <input style={{ ...S.input, fontSize: 15, padding: '12px 14px', marginTop: 6 }} placeholder="e.g. Hotel checkout, dinner, cab…" value={form.desc} onChange={e => setForm(f => ({ ...f, desc: e.target.value }))} />
          </div>
          <div style={{ marginBottom: '1.25rem' }}>
            <label style={S.label}>Category</label>
            <div style={{ display: 'flex', gap: 6, flexWrap: 'wrap', marginTop: 8 }}>
              {CATS.map(c => (
                <button key={c.id} onClick={() => setForm(f => ({ ...f, cat: c.id }))}
                  style={{ display: 'flex', alignItems: 'center', gap: 4, padding: '5px 10px', borderRadius: 18, fontSize: 12, border: `1.5px solid ${form.cat === c.id ? c.color : BRAND.borderLight}`, background: form.cat === c.id ? c.bg : BRAND.navyLight, color: form.cat === c.id ? c.color : BRAND.textMuted, cursor: 'pointer', fontFamily: "'DM Sans',sans-serif", fontWeight: form.cat === c.id ? 600 : 400 }}>
                  <span style={{ fontSize: 13 }}>{c.icon}</span>
                  <span>{c.label}</span>
                </button>
              ))}
          ...
          <div style={{ marginBottom: '1.5rem' }}>
            <label style={S.label}>Paid by & Split</label>
            <div style={{ marginTop: 8, background: BRAND.navyLight, borderRadius: 14, padding: '4px', border: `1px solid ${BRAND.borderLight}` }}>
              <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'center', gap: 8, padding: '12px 10px', flexWrap: 'wrap' }}>
                <span style={{ fontSize: 14, color: BRAND.textMuted }}>Paid by</span>
                <button onClick={() => setForm(f => ({ ...f, _paidByOpen: !f._paidByOpen, _splitOpen: false }))}
                  style={{ fontFamily: "'DM Sans',sans-serif", fontSize: 14, fontWeight: 600, color: BRAND.text, background: BRAND.navyCard, border: `1.5px solid ${BRAND.border}`, borderRadius: 10, padding: '6px 14px', cursor: 'pointer', display: 'flex', alignItems: 'center', gap: 6 }}>
                  <div style={{ width: 22, height: 22, borderRadius: '50%', background: mcolor(form.paidBy), display: 'flex', alignItems: 'center', justifyContent: 'center', color: '#fff', fontSize: 9, fontWeight: 700 }}>{form.paidBy.slice(0,2).toUpperCase()}</div>
                  {form.paidBy} <span style={{ fontSize: 10, color: BRAND.textMuted }}>▾</span>
                </button>
                <span style={{ fontSize: 14, color: BRAND.textMuted }}>and split</span>
                <button onClick={() => setForm(f => ({ ...f, _splitOpen: !f._splitOpen, _paidByOpen: false }))}
                  style={{ fontFamily: "'DM Sans',sans-serif", fontSize: 14, fontWeight: 600, color: BRAND.text, background: BRAND.navyCard, border: `1.5px solid ${BRAND.border}`, borderRadius: 10, padding: '6px 14px', cursor: 'pointer', display: 'flex', alignItems: 'center', gap: 6 }}>
                  {form.splitMode === 'all' ? 'equally' : `${form.splitWith.length} people`} <span style={{ fontSize: 10, color: BRAND.textMuted }}>▾</span>
                </button>
              </div>
              {form._paidByOpen && (
                <div style={{ borderTop: `1px solid ${BRAND.borderLight}`, padding: '8px 6px' }}>
                  {memberNames.map(m => (
                    <button key={m} onClick={() => setForm(f => ({ ...f, paidBy: m, _paidByOpen: false }))}
                      style={{ display: 'flex', alignItems: 'center', gap: 10, width: '100%', padding: '10px', borderRadius: 10, border: 'none', background: form.paidBy === m ? 'rgba(123,97,255,0.12)' : 'transparent', cursor: 'pointer', fontFamily: "'DM Sans',sans-serif", fontSize: 14, color: form.paidBy === m ? BRAND.purple : BRAND.text, marginBottom: 2 }}>
                      <div style={{ width: 32, height: 32, borderRadius: '50%', background: mcolor(m), display: 'flex', alignItems: 'center', justifyContent: 'center', color: '#fff', fontSize: 11, fontWeight: 700 }}>{m.slice(0,2).toUpperCase()}</div>
                      <span style={{ flex: 1, fontWeight: form.paidBy === m ? 600 : 400 }}>{m}</span>
                      {form.paidBy === m && <span style={{ fontSize: 16, color: BRAND.purple }}>✓</span>}
                    </button>
                  ))}
                </div>
              )}
              {form._splitOpen && (
                <div style={{ borderTop: `1px solid ${BRAND.borderLight}`, padding: '8px 6px' }}>
                  <button onClick={() => setForm(f => ({ ...f, splitMode: 'all', splitWith: [...memberNames] }))}
                    style={{ display: 'flex', alignItems: 'center', gap: 10, width: '100%', padding: '10px', borderRadius: 10, border: 'none', background: form.splitMode === 'all' ? 'rgba(123,97,255,0.12)' : 'transparent', cursor: 'pointer', fontFamily: "'DM Sans',sans-serif", fontSize: 14, color: form.splitMode === 'all' ? BRAND.purple : BRAND.text, marginBottom: 4 }}>
                    <div style={{ width: 32, height: 32, borderRadius: '50%', background: form.splitMode === 'all' ? BRAND.purple : BRAND.navyLight, display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: 16 }}>👥</div>
                    <span style={{ flex: 1 }}>Everyone equally</span>
                    <span style={{ fontSize: 12, color: BRAND.textMuted }}>÷{memberNames.length}</span>
                    <div style={{ width: 22, height: 22, borderRadius: 7, border: `2px solid ${form.splitMode === 'all' ? BRAND.purple : BRAND.borderLight}`, background: form.splitMode === 'all' ? BRAND.purple : 'transparent', display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: 12, color: '#fff' }}>{form.splitMode === 'all' ? '✓' : ''}</div>
                  </button>
                  {memberNames.map(m => {
                    const sel = form.splitWith.includes(m);
                    return (
                      <button key={m} onClick={() => setForm(f => {
                        ...
                        const isAll = newWith.length === memberNames.length;
                        return { ...f, splitMode: isAll ? 'all' : 'select', splitWith: newWith };
                      })} style={{ display: 'flex', alignItems: 'center', gap: 10, width: '100%', padding: '10px', borderRadius: 10, border: 'none', background: sel ? 'rgba(123,97,255,0.12)' : 'transparent', cursor: 'pointer', fontFamily: "'DM Sans',sans-serif", fontSize: 14, color: sel ? BRAND.purple : BRAND.text, marginBottom: 2 }}>
                        <div style={{ width: 32, height: 32, borderRadius: '50%', background: mcolor(m), display: 'flex', alignItems: 'center', justifyContent: 'center', color: '#fff', fontSize: 11, fontWeight: 700 }}>{m.slice(0,2).toUpperCase()}</div>
                        <span style={{ flex: 1, fontWeight: sel ? 600 : 400 }}>{m}</span>
                        {form.amount && parseFloat(form.amount) > 0 && sel && <span style={{ fontSize: 12, color: BRAND.textMuted }}>₹{(parseFloat(form.amount) / form.splitWith.length).toFixed(0)}</span>}
                        <div style={{ width: 22, height: 22, borderRadius: 7, border: `2px solid ${sel ? BRAND.purple : BRAND.borderLight}`, background: sel ? BRAND.purple : 'transparent', display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: 12, color: '#fff' }}>{sel ? '✓' : ''}</div>
                      </button>
                    );
                  })}
                </div>
  ...
  return (
    <div>
      {/* Hero */}
      <div style={{ background: 'linear-gradient(135deg,#0d1a3a,#1a1040,#0d1329)', borderRadius: 20, padding: '1.25rem 1.5rem', marginBottom: '1rem', position: 'relative', overflow: 'hidden', border: `1px solid rgba(123,97,255,0.15)` }}>
        <div style={{ position: 'absolute', top: -20, right: -20, fontSize: 90, opacity: 0.04 }}>₹</div>
        <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr 1fr', gap: 0, marginBottom: budget ? 14 : 0 }}>
          {[
            { label: 'Total spent', value: `₹${Math.round(total).toLocaleString('en-IN')}`, sub: `${expenses.length} expenses` },
            { label: 'Per person', value: `₹${Math.round(perPerson).toLocaleString('en-IN')}`, sub: 'equal share' },
            ...
            <div key={s.label} style={{ textAlign: i === 1 ? 'center' : i === 2 ? 'right' : 'left' }}>
              <div style={{ fontSize: 11, color: BRAND.textMuted, textTransform: 'uppercase', letterSpacing: '.05em', marginBottom: 3 }}>{s.label}</div>
              <div style={{ fontFamily: "'Poppins',sans-serif", fontSize: 22, fontWeight: 700, background: BRAND.gradient, WebkitBackgroundClip: 'text', WebkitTextFillColor: 'transparent', backgroundClip: 'text' }}>{s.value}</div>
              <div style={{ fontSize: 11, color: BRAND.textDim, marginTop: 2 }}>{s.sub}</div>
            </div>
          ))}
        </div>
        {budget && (
          <div style={{ marginTop: 14 }}>
            <div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: 5 }}>
              <span style={{ fontSize: 11, color: BRAND.textDim }}>{budgetPct}% of ₹{budget.toLocaleString('en-IN')} used</span>
              <span style={{ fontSize: 11, color: BRAND.textDim }}>₹{Math.round(Math.max(0, budgetLeft)).toLocaleString('en-IN')} left</span>
            </div>
            <div style={{ height: 5, background: 'rgba(255,255,255,0.06)', borderRadius: 4, overflow: 'hidden' }}>
              <div style={{ height: '100%', width: `${budgetPct}%`, background: budgetPct > 85 ? BRAND.danger : BRAND.gradientBtn, borderRadius: 4 }} />
            </div>
          </div>
        )}
        {!budget && <button onClick={() => setShowBudgetEdit(true)} style={{ ...S.btn, background: 'rgba(255,255,255,0.07)', color: BRAND.textMuted, border: `1px solid rgba(255,255,255,0.1)`, fontSize: 12, marginTop: 8 }}>+ Set a budget</button>}
        {budget && <button onClick={() => { setBudgetInput(String(budget)); setShowBudgetEdit(true); }} style={{ ...S.btn, background: 'rgba(255,255,255,0.06)', color: BRAND.textMuted, border: `1px solid rgba(255,255,255,0.08)`, fontSize: 11, marginTop: 8 }}>Edit budget</button>}
      </div>

      {showBudgetEdit && (
        <div style={{ ...S.card, border: `1px solid ${BRAND.border}`, marginBottom: '0.75rem' }}>
          <label style={S.label}>Total trip budget ₹</label>
          <input style={S.input} type="number" value={budgetInput} onChange={e => setBudgetInput(e.target.value)} placeholder="e.g. 50000" autoFocus />
          <div style={{ display: 'flex', gap: 8, marginTop: 10 }}>
            <button style={{ ...S.btn, ...S.btnP, flex: 1, justifyContent: 'center', padding: '9px' }} onClick={async () => { const v = parseFloat(budgetInput); if (!isNaN(v) && v > 0) { setLocalBudget(v); try { const { updateTrip } = await import('./api'); await updateTrip(trip.id, { budget: v }); } catch (_) {} } setShowBudgetEdit(false); }}>✓ Save</button>
            {budget && <button style={{ ...S.btn, ...S.btnDanger }} onClick={async () => { setLocalBudget(null); try { const { updateTrip } = await import('./api'); await updateTrip(trip.id, { budget: null }); } catch (_) {} setShowBudgetEdit(false); }}>Remove</button>}
            <button style={S.btn} onClick={() => setShowBudgetEdit(false)}>✕</button>
          </div>
        </div>
      )}
      ...
      <div style={{ display: 'flex', gap: 6, flexWrap: 'wrap', marginBottom: '1rem' }}>
        {memberNames.map(m => (
          <div key={m} style={{ display: 'flex', alignItems: 'center', gap: 6, background: BRAND.navyCard, border: `1px solid ${BRAND.borderLight}`, borderRadius: 20, padding: '4px 12px 4px 5px', fontSize: 13 }}>
            <div style={{ width: 22, height: 22, borderRadius: '50%', background: mcolor(m), display: 'flex', alignItems: 'center', justifyContent: 'center', color: '#fff', fontSize: 10, fontWeight: 700 }}>{m.slice(0, 2).toUpperCase()}</div>
            {m}
          </div>
        ))}
      ...
      {/* Section tabs */}
      <div style={{ display: 'flex', gap: 0, background: BRAND.navyCard, border: `1px solid ${BRAND.borderLight}`, borderRadius: 13, padding: 3, marginBottom: '1rem' }}>
        {SECTION_TABS.map(t => (
          <button key={t.id} onClick={() => setSection(t.id)}
            style={{ flex: 1, padding: '8px 4px', fontSize: 12, fontWeight: section === t.id ? 600 : 400, borderRadius: 10, border: 'none', cursor: 'pointer', fontFamily: "'DM Sans',sans-serif", background: section === t.id ? BRAND.gradientBtn : 'transparent', color: section === t.id ? '#fff' : BRAND.textMuted }}>
            {t.label}
          </button>
        ))}
      </div>
        ...
        <div style={{ paddingBottom: '5rem' }}>
          <div style={{ display: 'flex', gap: 5, flexWrap: 'wrap', marginBottom: '0.75rem' }}>
            {[{ id: 'all', label: 'All', icon: '' }, ...CATS.filter(c => catTotals[c.id] > 0)].map(c => {
              ...
              return (
                <button key={c.id} onClick={() => setFilterCat(c.id)}
                  style={{ ...S.btn, fontSize: 11, padding: '4px 10px', borderRadius: 16, background: filterCat === c.id ? (c.id === 'all' ? BRAND.purple : cat?.bg || BRAND.navyLight) : BRAND.navyLight, color: filterCat === c.id ? (c.id === 'all' ? '#fff' : cat?.color || BRAND.text) : BRAND.textMuted, border: `1px solid ${filterCat === c.id ? (c.id === 'all' ? BRAND.purple : (cat?.color || BRAND.purple) + '44') : BRAND.borderLight}` }}>
                  {c.icon} {c.label || 'All'}
                </button>
              );
            })}
          </div>
          {sortedExpenses.length === 0 && <div style={{ textAlign: 'center', padding: '4rem 1rem', color: BRAND.textMuted }}><div style={{ fontSize: 48, marginBottom: 14 }}>🧾</div><p style={{ fontSize: 15, fontWeight: 500, marginBottom: 6 }}>No expenses yet</p><p style={{ fontSize: 13 }}>Tap + to add your first one</p></div>}
          {sortedExpenses.map(exp => {
            const cat = CATS.find(c => c.id === exp.cat) || CATS[4];
            const splitArr = Array.isArray(exp.split) && exp.split.length > 0 ? exp.split : memberNames;
            return (
              <div key={exp.id} style={{ ...S.card, display: 'flex', alignItems: 'center', gap: 12, marginBottom: 8, borderLeft: `3px solid ${cat.color}`, borderRadius: '0 14px 14px 0', padding: '12px 14px 12px 12px' }}>
                <div style={{ width: 42, height: 42, borderRadius: 12, display: 'flex', alignItems: 'center', justifyContent: 'center', background: cat.bg, flexShrink: 0, fontSize: 20 }}>{cat.icon}</div>
                <div style={{ flex: 1, minWidth: 0 }}>
                  <div style={{ fontSize: 14, fontWeight: 500, marginBottom: 2 }}>{exp.desc}</div>
                  <div style={{ display: 'flex', alignItems: 'center', gap: 6, flexWrap: 'wrap' }}>
                    <div style={{ display: 'flex', alignItems: 'center', gap: 4 }}>
                      <div style={{ width: 16, height: 16, borderRadius: '50%', background: mcolor(exp.paidBy), display: 'flex', alignItems: 'center', justifyContent: 'center', color: '#fff', fontSize: 8, fontWeight: 700 }}>{exp.paidBy.slice(0, 2).toUpperCase()}</div>
                      <span style={{ fontSize: 12, color: BRAND.textMuted }}>{exp.paidBy}</span>
                    </div>
                    <span style={{ fontSize: 11, color: BRAND.textDim }}>·</span>
                    <div style={{ display: 'flex' }}>
                      {splitArr.slice(0, 4).map((m, i) => (
                        <div key={m} style={{ width: 16, height: 16, borderRadius: '50%', background: mcolor(m), border: `1.5px solid ${BRAND.navyCard}`, marginLeft: i > 0 ? -5 : 0 }} />
                      ))}
                    </div>
                    <span style={{ fontSize: 11, color: BRAND.textDim }}>·</span>
                    <span style={{ fontSize: 11, color: BRAND.textDim }}>{new Date(exp.date).toLocaleDateString('en-IN', { day: 'numeric', month: 'short' })}</span>
                  </div>
                </div>
                <div style={{ textAlign: 'right', flexShrink: 0 }}>
                  <div style={{ fontFamily: "'Poppins',sans-serif", fontSize: 16, fontWeight: 700, background: BRAND.gradient, WebkitBackgroundClip: 'text', WebkitTextFillColor: 'transparent', backgroundClip: 'text' }}>₹{Math.round(exp.amount).toLocaleString('en-IN')}</div>
                  <div style={{ fontSize: 11, color: BRAND.textMuted, marginTop: 2 }}>₹{Math.round(exp.amount / splitArr.length).toLocaleString('en-IN')} each</div>
                  <button onClick={() => handleDelete(exp.id)} style={{ ...S.btn, padding: '2px 6px', fontSize: 11, color: BRAND.textDim, border: 'none', background: 'transparent' }}>✕</button>
                </div>
              </div>
            );
          })}
      ...
      {section === 'shares' && (
        <div>
          <div style={{ ...S.card, background: 'linear-gradient(135deg, rgba(123,97,255,0.1), rgba(255,106,174,0.06))', border: `1px solid ${BRAND.border}`, marginBottom: '1rem' }}>
            <div style={{ fontFamily: "'Poppins',sans-serif", fontSize: 28, fontWeight: 700, background: BRAND.gradient, WebkitBackgroundClip: 'text', WebkitTextFillColor: 'transparent', backgroundClip: 'text' }}>₹{Math.round(total).toLocaleString('en-IN')}</div>
            <div style={{ fontSize: 12, color: BRAND.textMuted, marginTop: 3 }}>{memberNames.length} members · {expenses.length} expenses · {days} days</div>
          </div>
          <div style={{ ...S.card, marginBottom: '0.75rem' }}>
            {memberNames.map((m, i) => {
              const paid = expenses.filter(e => e.paidBy === m).reduce((s, e) => s + e.amount, 0);
              const owes = expenses.reduce((s, e) => { const sp = Array.isArray(e.split) && e.split.length > 0 ? e.split : memberNames; return sp.includes(m) ? s + e.amount / sp.length : s; }, 0);
              const net = paid - owes;
              return (
                <div key={m} style={{ display: 'flex', alignItems: 'center', gap: 10, padding: '10px 0', borderBottom: i < memberNames.length - 1 ? `1px solid ${BRAND.borderLight}` : 'none' }}>
                  <div style={{ width: 34, height: 34, borderRadius: '50%', background: mcolor(m), display: 'flex', alignItems: 'center', justifyContent: 'center', color: '#fff', fontSize: 12, fontWeight: 700, flexShrink: 0 }}>{m.slice(0, 2).toUpperCase()}</div>
                  <div style={{ flex: 1 }}>
                    <div style={{ fontSize: 14, fontWeight: 500 }}>{m}</div>
                    <div style={{ fontSize: 11, color: BRAND.textMuted, marginTop: 1 }}>paid ₹{Math.round(paid).toLocaleString('en-IN')} · share ₹{Math.round(owes).toLocaleString('en-IN')}</div>
                  </div>
                  <div style={{ textAlign: 'right' }}>
                    <div style={{ fontFamily: "'Poppins',sans-serif", fontSize: 16, fontWeight: 700, color: net >= 0 ? BRAND.success : BRAND.danger }}>{net >= 0 ? '+' : '−'}₹{Math.abs(Math.round(net)).toLocaleString('en-IN')}</div>
                    <div style={{ fontSize: 11, color: BRAND.textMuted }}>{net > 0.5 ? 'gets back' : net < -0.5 ? 'owes' : 'settled'}</div>
                  </div>
                </div>
              );
            })}
          </div>
          <div style={{ fontSize: 12, fontWeight: 600, color: BRAND.textMuted, textTransform: 'uppercase', letterSpacing: '.04em', marginBottom: 8 }}>Who pays whom</div>
          {settlements.length === 0
            ? <div style={{ background: BRAND.successBg, border: `1px solid ${BRAND.successBorder}`, borderRadius: 12, padding: '1rem 1.25rem', fontSize: 14, color: BRAND.success, fontWeight: 500 }}>✅ Everyone is squared up!</div>
            : settlements.map((s, i) => (
              <div key={i} style={{ ...S.card, display: 'flex', alignItems: 'center', gap: 10, marginBottom: 8 }}>
                <div style={{ width: 30, height: 30, borderRadius: '50%', background: mcolor(s.from), display: 'flex', alignItems: 'center', justifyContent: 'center', color: '#fff', fontSize: 11, fontWeight: 700 }}>{s.from.slice(0, 2).toUpperCase()}</div>
                <span style={{ fontSize: 13, fontWeight: 500 }}>{s.from}</span>
                <div style={{ flex: 1, display: 'flex', alignItems: 'center', gap: 4 }}>
                  <div style={{ height: 1, flex: 1, background: BRAND.borderLight }} />
                  <span style={{ fontSize: 11, color: BRAND.orange, padding: '2px 6px', background: 'rgba(255,138,61,0.12)', borderRadius: 8, fontWeight: 600 }}>→</span>
                  <div style={{ height: 1, flex: 1, background: BRAND.borderLight }} />
                </div>
                <span style={{ fontSize: 13, fontWeight: 500 }}>{s.to}</span>
                <div style={{ width: 30, height: 30, borderRadius: '50%', background: mcolor(s.to), display: 'flex', alignItems: 'center', justifyContent: 'center', color: '#fff', fontSize: 11, fontWeight: 700 }}>{s.to.slice(0, 2).toUpperCase()}</div>
                <div style={{ fontFamily: "'Poppins',sans-serif", fontSize: 16, fontWeight: 700, color: BRAND.success, marginLeft: 6 }}>₹{Math.round(s.amt).toLocaleString('en-IN')}</div>
              </div>
            ))
          }
        </div>
      ...
      {section === 'balances' && (
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill,minmax(150px,1fr))', gap: 10 }}>
          {memberNames.map(m => {
            ...
            const isPos = b >= 0.5, isNeg = b < -0.5;
            return (
              <div key={m} style={{ ...S.card, borderTop: `3px solid ${isPos ? BRAND.success : isNeg ? BRAND.danger : BRAND.borderLight}`, borderRadius: '0 0 14px 14px', padding: '1rem' }}>
                <div style={{ display: 'flex', alignItems: 'center', gap: 8, marginBottom: 10 }}>
                  <div style={{ width: 32, height: 32, borderRadius: '50%', background: mcolor(m), display: 'flex', alignItems: 'center', justifyContent: 'center', color: '#fff', fontSize: 12, fontWeight: 700 }}>{m.slice(0, 2).toUpperCase()}</div>
                  <div style={{ fontSize: 14, fontWeight: 500 }}>{m}</div>
                </div>
                <div style={{ fontFamily: "'Poppins',sans-serif", fontSize: 22, fontWeight: 700, color: isPos ? BRAND.success : isNeg ? BRAND.danger : BRAND.textMuted }}>
                  {isPos ? '+' : ''}₹{Math.abs(Math.round(b)).toLocaleString('en-IN')}
                </div>
                <div style={{ fontSize: 11, color: BRAND.textMuted, marginTop: 4 }}>{isPos ? 'gets back' : isNeg ? 'owes' : 'all settled'}</div>
              </div>
            );
          })}
        </div>
        ...
        <div>
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(3,1fr)', gap: 8, marginBottom: 10 }}>
            {[
              ...
              <div key={s.label} style={{ background: s.warn ? BRAND.dangerBg : BRAND.navyLight, borderRadius: 12, padding: '10px 12px', border: s.warn ? `1px solid ${BRAND.dangerBorder}` : `1px solid ${BRAND.borderLight}` }}>
                <div style={{ fontSize: 11, color: s.warn ? BRAND.danger : BRAND.textMuted, marginBottom: 3 }}>{s.label}</div>
                <div style={{ fontFamily: "'Poppins',sans-serif", fontSize: 18, fontWeight: 700, color: s.warn ? BRAND.danger : BRAND.text }}>{s.value}</div>
                <div style={{ fontSize: 11, color: s.warn ? BRAND.orange : BRAND.textDim, marginTop: 2 }}>{s.sub}</div>
              </div>
            ))}
          </div>
          {budget && (
            <div style={{ ...S.card, display: 'flex', flexDirection: 'column', alignItems: 'center', padding: '1.25rem 1rem', marginBottom: 10 }}>
              <div style={{ fontSize: 11, color: BRAND.textMuted, textTransform: 'uppercase', letterSpacing: '.05em', marginBottom: 12 }}>Budget health</div>
              <div style={{ position: 'relative', width: 130, height: 130 }}>
                <canvas ref={donutRef} />
              </div>
            </div>
          ...
          {top3.length > 0 && (
            <div style={{ ...S.card, marginBottom: 10 }}>
              <div style={{ fontSize: 11, color: BRAND.textMuted, textTransform: 'uppercase', letterSpacing: '.05em', marginBottom: 12 }}>Top expenses</div>
              {top3.map((exp, idx) => {
                const cat = CATS.find(c => c.id === exp.cat) || CATS[4];
                const pct = total > 0 ? Math.round(exp.amount / total * 100) : 0;
                return (
                  <div key={exp.id} style={{ display: 'flex', alignItems: 'center', gap: 10, padding: idx < top3.length - 1 ? '0 0 10px' : '0', borderBottom: idx < top3.length - 1 ? `1px solid ${BRAND.borderLight}` : 'none', marginBottom: idx < top3.length - 1 ? 10 : 0 }}>
                    <span style={{ fontSize: 18, flexShrink: 0 }}>{['🥇','🥈','🥉'][idx]}</span>
                    <div style={{ width: 34, height: 34, borderRadius: 9, background: cat.bg, display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: 16, flexShrink: 0 }}>{cat.icon}</div>
                    <div style={{ flex: 1, minWidth: 0 }}>
                      <div style={{ fontSize: 13, fontWeight: 500 }}>{exp.desc}</div>
                      <div style={{ height: 4, background: BRAND.navyLight, borderRadius: 4, overflow: 'hidden', marginTop: 5 }}>
                        <div style={{ height: '100%', width: `${pct}%`, background: cat.color, borderRadius: 4 }} />
                      </div>
                    </div>
                    <div style={{ textAlign: 'right', flexShrink: 0 }}>
                      <div style={{ fontFamily: "'Poppins',sans-serif", fontSize: 14, fontWeight: 700 }}>₹{Math.round(exp.amount).toLocaleString('en-IN')}</div>
                      <div style={{ fontSize: 11, color: BRAND.textMuted }}>{pct}%</div>
                    </div>
                  </div>
                );
              })}
          ...
          {Object.values(catTotals).some(v => v > 0) && (
            <div style={{ ...S.card, marginBottom: 10 }}>
              <div style={{ fontSize: 11, color: BRAND.textMuted, textTransform: 'uppercase', letterSpacing: '.05em', marginBottom: 12 }}>Category breakdown</div>
              <div style={{ position: 'relative', height: 160 }}>
                <canvas ref={barRef} />
              </div>
            </div>
...
function PhotosPage({ trip, myNickname }) {
  ...
  const byMember = useMemo(() => {
    const map = {};
    memberNames.forEach(m => { map[m] = []; });
    allPhotos.forEach(p => { if (map[p.uploader]) map[p.uploader].push(p); else map[p.uploader] = [p]; });
    return map;
  }, [allPhotos, memberNames]);
  ...
  const processFiles = async (files) => {
    ...
    for (let i = 0; i < files.length; i++) {
      ...
      try {
        const res = await addPhoto(trip.id, publicUrl);
        setAllPhotos(p => [...p, res.photo || { id: Date.now() + Math.random(), url: publicUrl, uploader: me }]);
      } catch { setAllPhotos(p => [...p, { id: Date.now() + Math.random(), url: publicUrl, uploader: me }]); }
      setUploadProgress(Math.round(((i + 1) / files.length) * 100));
    }
    setUploading(false); setUploadProgress(0); setActiveFolder(me);
  };

  const handleUpload = (e) => processFiles(Array.from(e.target.files));
  const handleDrop = (e) => { e.preventDefault(); setDragging(false); const files = Array.from(e.dataTransfer.files).filter(f => f.type.startsWith('image/')); if (files.length) processFiles(files); };
  ...
  const doDeleteSingle = async (photo) => {
    ...
    setAllPhotos(p => p.filter(x => x.id !== photo.id));
    setSelected(s => { const n = new Set(s); n.delete(photo.id); return n; });
    setPendingDeletePhoto(null); setConfirmDelete(null);
  };
  ...
  const confirmDeleteAction = () => { if (confirmDelete === 'single' && pendingDeletePhoto) doDeleteSingle(pendingDeletePhoto); else if (confirmDelete === 'bulk') doDeleteBulk(); };
  ...
  const downloadSelected = async () => {
    ...
    for (const p of photos) {
      try {
        ...
        const blobUrl = URL.createObjectURL(blob);
        const a = document.createElement('a'); a.href = blobUrl; a.download = p.url.split('/').pop() || `photo-${p.id}.jpg`;
  ...
  useEffect(() => {
    const onKey = (e) => {
      if (confirmDelete) { if (e.key === 'Escape') cancelDelete(); if (e.key === 'Enter') confirmDeleteAction(); return; }
      if (!lightbox) return;
      if (e.key === 'ArrowLeft') lbPrev();
      if (e.key === 'ArrowRight') lbNext();
      if (e.key === 'Escape') setLightbox(null);
  ...
  return (
    <div style={{ background: BRAND.navy, minHeight: '100vh', paddingBottom: '7rem', fontFamily: "'DM Sans',sans-serif" }}>
      {/* Folder tabs */}
      <div style={{ display: 'flex', gap: 12, overflowX: 'auto', padding: '1.25rem 1.25rem 1rem', scrollbarWidth: 'none' }}>
        {memberNames.map(m => {
          const count = (byMember[m] || []).length;
          const isActive = activeFolder === m;
          const isMe = m === me;
          return (
            <div key={m} onClick={() => { setActiveFolder(m); setSelected(new Set()); }}
              style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', gap: 7, cursor: 'pointer', flexShrink: 0 }}>
              <div style={{ width: 62, height: 54, borderRadius: 14, position: 'relative', display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: 15, fontWeight: 700, background: isActive ? BRAND.gradientBtn : BRAND.navyCard, border: `1.5px solid ${isActive ? 'transparent' : BRAND.borderLight}`, color: isActive ? '#fff' : BRAND.textMuted, transition: 'all .2s', boxShadow: isActive ? '0 6px 20px rgba(123,97,255,0.35)' : 'none', transform: isActive ? 'translateY(-2px)' : 'none' }}>
                {isMe ? '👤' : initials(m)}
                {count > 0 && <span style={{ position: 'absolute', top: -6, right: -6, background: BRAND.purple, color: '#fff', fontSize: 9, fontWeight: 700, minWidth: 18, height: 18, borderRadius: 9, display: 'flex', alignItems: 'center', justifyContent: 'center', padding: '0 4px', border: `2px solid ${BRAND.navy}` }}>{count}</span>}
              </div>
              <span style={{ fontSize: 10, fontWeight: 500, color: isActive ? BRAND.text : BRAND.textDim, maxWidth: 66, textAlign: 'center', whiteSpace: 'nowrap', overflow: 'hidden', textOverflow: 'ellipsis' }}>{isMe ? 'Mine' : m}</span>
            </div>
          );
        })}
      </div>

      <div style={{ height: 1, background: `linear-gradient(90deg, transparent, ${BRAND.borderLight}, transparent)`, margin: '0 1.25rem' }} />

      {/* Section header */}
      <div style={{ display: 'flex', alignItems: 'baseline', gap: 10, padding: '1.25rem 1.25rem 1rem' }}>
        <h2 style={{ fontFamily: "'Poppins',sans-serif", fontSize: 22, fontWeight: 700, margin: 0, color: BRAND.text }}>
          {isMyFolder ? <>Your <span style={{ background: BRAND.gradient, WebkitBackgroundClip: 'text', WebkitTextFillColor: 'transparent', backgroundClip: 'text' }}>shots</span></> : <><span style={{ background: BRAND.gradient, WebkitBackgroundClip: 'text', WebkitTextFillColor: 'transparent', backgroundClip: 'text' }}>{activeFolder}</span>'s shots</>}
        </h2>
        <span style={{ fontSize: 11, color: BRAND.textDim }}>{folderPhotos.length} photo{folderPhotos.length !== 1 ? 's' : ''}</span>
      </div>

      {/* Upload zone or view banner */}
      {isMyFolder ? (
        <label onDragOver={(e) => { e.preventDefault(); setDragging(true); }} onDragLeave={() => setDragging(false)} onDrop={handleDrop}
          style={{ display: 'block', margin: '0 1.25rem 1.25rem', borderRadius: 18, padding: '2rem 1.5rem', textAlign: 'center', cursor: 'pointer', position: 'relative', overflow: 'hidden', background: dragging ? 'rgba(123,97,255,0.08)' : BRAND.navyCard, border: `1.5px dashed ${dragging ? BRAND.purple : BRAND.borderLight}`, transition: 'all .3s' }}>
          <input type="file" accept="image/*" multiple style={{ display: 'none' }} onChange={handleUpload} />
          {uploading && (
            <div style={{ position: 'absolute', inset: 0, background: 'rgba(13,19,41,0.85)', backdropFilter: 'blur(4px)', borderRadius: 16, display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', gap: 10 }}>
              <div style={{ width: 32, height: 32, border: `3px solid rgba(123,97,255,0.2)`, borderTopColor: BRAND.purple, borderRadius: '50%', animation: 'spin .7s linear infinite' }} />
              <div style={{ fontSize: 12, color: BRAND.purple, fontWeight: 600 }}>Uploading… {uploadProgress}%</div>
              <div style={{ width: '60%', height: 4, background: 'rgba(255,255,255,0.06)', borderRadius: 4, overflow: 'hidden' }}>
                <div style={{ height: '100%', width: `${uploadProgress}%`, background: BRAND.gradientBtn, borderRadius: 4 }} />
              </div>
            </div>
          )}
          <span style={{ fontSize: 40, marginBottom: 12, display: 'block', animation: 'bob 3s ease-in-out infinite' }}>{dragging ? '🎯' : '📷'}</span>
          <p style={{ fontSize: 14, fontWeight: 500, color: BRAND.textMuted, margin: '0 0 5px' }}>{dragging ? 'Release to upload' : 'Drop photos or tap to pick'}</p>
          <p style={{ fontSize: 12, color: BRAND.textDim, margin: 0 }}>JPG · PNG · HEIC · WebP</p>
          <div style={{ display: 'inline-flex', alignItems: 'center', gap: 6, background: 'rgba(123,97,255,0.1)', border: `1px solid ${BRAND.border}`, color: BRAND.purple, fontSize: 11, fontWeight: 600, padding: '4px 12px', borderRadius: 99, marginTop: 12 }}>👤 {me}</div>
        </label>
      ) : (
        <div style={{ margin: '0 1.25rem 1.25rem', background: BRAND.navyCard, border: `1px solid ${BRAND.borderLight}`, borderRadius: 16, padding: '.9rem 1.25rem', display: 'flex', alignItems: 'center', gap: 12 }}>
          <div style={{ width: 38, height: 38, borderRadius: '50%', background: BRAND.navyLight, display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: 13, fontWeight: 700, color: BRAND.textMuted, border: `1.5px solid ${BRAND.borderLight}` }}>{initials(activeFolder)}</div>
          <div style={{ fontSize: 13, color: BRAND.textMuted, lineHeight: 1.5 }}>Viewing <strong style={{ color: BRAND.text }}>{activeFolder}'s</strong> collection — tap to select, then download.</div>
        </div>
      )}

      {/* Photo grid */}
      {folderPhotos.length === 0 ? (
        <div style={{ textAlign: 'center', padding: '4rem 1.25rem', color: BRAND.textDim }}>
          <span style={{ fontSize: 56, marginBottom: 16, display: 'block', opacity: .4 }}>{isMyFolder ? '🌄' : '🫙'}</span>
          <p style={{ fontSize: 16, fontWeight: 500, color: BRAND.textMuted }}>{isMyFolder ? 'Your roll is empty' : `${activeFolder} hasn't shared yet`}</p>
          <p style={{ fontSize: 13 }}>{isMyFolder ? 'Tap above to add your first memory' : 'Check back soon!'}</p>
        </div>
      ) : (
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill,minmax(110px,1fr))', gap: 4, padding: '0 1.25rem' }}>
          {folderPhotos.map((p, idx) => (
            <div key={p.id} onClick={() => toggle(p.id)}
              style={{ position: 'relative', borderRadius: 10, overflow: 'hidden', aspectRatio: '1', cursor: 'pointer', background: BRAND.navyCard, border: `2px solid ${selected.has(p.id) ? BRAND.purple : 'transparent'}`, boxShadow: selected.has(p.id) ? `0 0 0 2px rgba(123,97,255,0.3)` : 'none' }}>
              <img src={p.url} alt="" loading="lazy" style={{ width: '100%', height: '100%', objectFit: 'cover', display: 'block' }} onError={e => { e.target.style.display = 'none'; }} />
              <div style={{ position: 'absolute', top: 7, right: 7, width: 22, height: 22, borderRadius: '50%', background: selected.has(p.id) ? BRAND.purple : 'rgba(0,0,0,0.45)', border: `2px solid ${selected.has(p.id) ? BRAND.purple : 'rgba(255,255,255,0.3)'}`, display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: 11, color: '#fff', backdropFilter: 'blur(4px)' }}>
                {selected.has(p.id) ? '✓' : ''}
              </div>
              {isMyFolder && (
                <button onClick={(e) => askDeleteSingle(p, e)} title="Delete photo"
                  style={{ position: 'absolute', top: 7, left: 7, width: 26, height: 26, borderRadius: '50%', background: 'rgba(0,0,0,0.55)', border: 'none', color: '#fff', fontSize: 12, cursor: 'pointer', display: 'flex', alignItems: 'center', justifyContent: 'center', opacity: 0, transition: 'opacity .15s', backdropFilter: 'blur(4px)' }}
                  onMouseEnter={e => e.currentTarget.style.opacity = '1'}
                  onMouseLeave={e => e.currentTarget.style.opacity = '0'}>
                  🗑
                </button>
              )}
              <div style={{ position: 'absolute', inset: 0, display: 'flex', alignItems: 'center', justifyContent: 'center', opacity: 0, transition: 'opacity .2s', background: 'rgba(0,0,0,0.3)' }}
                onClick={(e) => { e.stopPropagation(); openLightbox(idx); }}
                onMouseEnter={e => e.currentTarget.style.opacity = '1'}
                onMouseLeave={e => e.currentTarget.style.opacity = '0'}>
                <div style={{ background: 'rgba(0,0,0,0.6)', backdropFilter: 'blur(6px)', borderRadius: '50%', width: 36, height: 36, display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: 16, border: '1px solid rgba(255,255,255,0.15)' }}>⛶</div>
              </div>
            </div>
          ))}
        </div>
      ...
      {selected.size > 0 && (
        <div style={{ position: 'fixed', bottom: 0, left: '50%', transform: 'translateX(-50%)', width: '100%', maxWidth: 880, background: 'rgba(13,19,41,0.96)', backdropFilter: 'blur(24px)', borderTop: `1px solid ${BRAND.borderLight}`, padding: '14px 1.25rem', display: 'flex', alignItems: 'center', gap: 10, zIndex: 190 }}>
          <div style={{ flex: 1, fontSize: 13, color: BRAND.textMuted }}><strong style={{ color: BRAND.text, fontSize: 15 }}>{selected.size}</strong> selected</div>
          <button onClick={clearSel} style={{ ...S.btn, fontSize: 13 }}>Clear</button>
          <button onClick={downloadSelected} style={{ ...S.btn, ...S.btnP, display: 'flex', alignItems: 'center', gap: 7 }}>⬇ Download <span style={{ background: 'rgba(255,255,255,0.2)', borderRadius: 99, padding: '1px 7px', fontSize: 12, fontWeight: 700 }}>{selected.size}</span></button>
          {isMyFolder && <button onClick={askDeleteBulk} style={{ ...S.btn, background: BRAND.dangerBg, color: BRAND.danger, border: `1px solid ${BRAND.dangerBorder}`, display: 'flex', alignItems: 'center', gap: 7 }}>🗑 Delete <span style={{ background: BRAND.dangerBg, borderRadius: 99, padding: '1px 7px', fontSize: 12, fontWeight: 700, color: BRAND.danger }}>{selected.size}</span></button>}
        </div>
      )}

      {/* Confirm dialog */}
      {confirmDelete && (
        <div style={{ position: 'fixed', inset: 0, background: 'rgba(0,0,0,0.8)', backdropFilter: 'blur(12px)', zIndex: 700, display: 'flex', alignItems: 'center', justifyContent: 'center', padding: '1.25rem' }}>
          <div style={{ background: BRAND.navyMid, border: `1px solid ${BRAND.borderLight}`, borderRadius: 20, padding: '1.75rem 1.5rem 1.5rem', width: '100%', maxWidth: 340, boxShadow: '0 32px 80px rgba(0,0,0,0.8)' }}>
            <span style={{ fontSize: 36, display: 'block', textAlign: 'center', marginBottom: 14 }}>🗑️</span>
            <h3 style={{ fontFamily: "'Poppins',sans-serif", fontSize: 20, color: BRAND.text, textAlign: 'center', margin: '0 0 8px' }}>{confirmDelete === 'bulk' ? `Delete ${selected.size} photo${selected.size > 1 ? 's' : ''}?` : 'Delete this photo?'}</h3>
            <p style={{ fontSize: 13, color: BRAND.textMuted, textAlign: 'center', margin: '0 0 1.5rem', lineHeight: 1.55 }}>This will be permanently removed. This can't be undone.</p>
            <div style={{ display: 'flex', gap: 10 }}>
              <button onClick={cancelDelete} style={{ ...S.btn, flex: 1, justifyContent: 'center', padding: '11px' }}>Cancel</button>
              <button onClick={confirmDeleteAction} style={{ flex: 1, background: 'linear-gradient(135deg,#c0392b,#922b21)', border: 'none', color: '#fff', fontSize: 14, fontFamily: "'DM Sans',sans-serif", fontWeight: 600, padding: '11px', borderRadius: 12, cursor: 'pointer' }}>{confirmDelete === 'bulk' ? `Delete ${selected.size}` : 'Delete'}</button>
            </div>
          </div>
        </div>
      )}
      ...
      {/* Lightbox */}
      {lightbox && (
        <div style={{ position: 'fixed', inset: 0, background: 'rgba(0,0,0,0.97)', zIndex: 600, display: 'flex', alignItems: 'center', justifyContent: 'center', flexDirection: 'column' }} onClick={() => setLightbox(null)}>
          <button style={{ position: 'absolute', top: 16, right: 16, background: 'rgba(255,255,255,0.07)', border: `1px solid rgba(255,255,255,0.1)`, color: BRAND.text, width: 38, height: 38, borderRadius: '50%', display: 'flex', alignItems: 'center', justifyContent: 'center', cursor: 'pointer', fontSize: 16 }} onClick={() => setLightbox(null)}>✕</button>
          <img src={lightbox.photos[lightbox.index]?.url} alt="" style={{ maxWidth: '92vw', maxHeight: '78vh', objectFit: 'contain', borderRadius: 10 }} onClick={(e) => e.stopPropagation()} />
          <div style={{ display: 'flex', alignItems: 'center', gap: 20, marginTop: 20 }} onClick={(e) => e.stopPropagation()}>
            <button onClick={lbPrev} disabled={lightbox.index === 0} style={{ background: 'rgba(255,255,255,0.07)', border: `1px solid rgba(255,255,255,0.1)`, color: BRAND.text, width: 42, height: 42, borderRadius: '50%', display: 'flex', alignItems: 'center', justifyContent: 'center', cursor: 'pointer', fontSize: 18, opacity: lightbox.index === 0 ? 0.2 : 1 }}>‹</button>
            <span style={{ fontSize: 12, color: BRAND.textDim, minWidth: 55, textAlign: 'center' }}>{lightbox.index + 1} / {lightbox.photos.length}</span>
            <button onClick={lbNext} disabled={lightbox.index === lightbox.photos.length - 1} style={{ background: 'rgba(255,255,255,0.07)', border: `1px solid rgba(255,255,255,0.1)`, color: BRAND.text, width: 42, height: 42, borderRadius: '50%', display: 'flex', alignItems: 'center', justifyContent: 'center', cursor: 'pointer', fontSize: 18, opacity: lightbox.index === lightbox.photos.length - 1 ? 0.2 : 1 }}>›</button>
          </div>
        </div>
      )}

      <style>{`@keyframes bob{0%,100%{transform:translateY(0) rotate(-3deg)}50%{transform:translateY(-6px) rotate(3deg)}}`}</style>
    </div>
  );
}
...
/* ═══════════════════════════════════════════════════════
   ITINERARY PAGE
═══════════════════════════════════════════════════════ */
const SLOT_LABELS = { morning: '6AM–12PM', afternoon: '12PM–6PM', evening: '6PM–12AM' };
const SLOT_ORDER = ['morning', 'afternoon', 'evening'];
const MONTH_NAMES = ['Jan','Feb','Mar','Apr','May','Jun','Jul','Aug','Sep','Oct','Nov','Dec'];
...
function ItineraryPage({ trip }) {
  const isSolo = trip.isSolo;
  const [iTab, setITab] = useState('planner');
  const [form] = useState({ dest: trip.destination || '', arrival: trip.arrival ? new Date(trip.arrival).toISOString().split('T')[0] : '', departure: trip.departure ? new Date(trip.departure).toISOString().split('T')[0] : '', arrivalSlot: trip.arrivalSlot || 'morning', departureSlot: trip.departureSlot || 'morning', budget: trip.budget ? String(trip.budget) : '', people: String(normalizeMembers(trip.members).length || 1) });
  const days = form.arrival && form.departure ? Math.max(1, Math.round((new Date(form.departure) - new Date(form.arrival)) / 86400000)) : 1;
  const [step, setStep] = useState(trip._cachedItin ? 'result' : 'loading');
  const [itin, setItin] = useState(trip._cachedItin?.itinerary || null);
  const [sources, setSources] = useState(trip._cachedItin?.sources || []);
  const [localTasteData, setLocalTasteData] = useState(trip._cachedTaste || null);
  ...
  const TYPE_ICONS = { attraction: '🏛️', food: '🍽️', experience: '✨', transport: '🚗', hotel: '🏨', shopping: '🛍️' };
  const firstActivitySlot = () => { const idx = SLOT_ORDER.indexOf(form.arrivalSlot); return SLOT_ORDER[Math.min(idx + 1, SLOT_ORDER.length - 1)]; };
  ...
  const runGenerateItinerary = async () => {
    ...
    try {
      const { generateItinerary } = await import('./api');
      const result = await generateItinerary({ destination: form.dest, days, budget: form.budget ? parseFloat(form.budget) : null, people: parseInt(form.people) || 1, interests: ['🛕 Temples', '🍽️ Food', '🛍️ Shopping'], arrivalSlot: form.arrivalSlot, departureSlot: form.departureSlot, firstActivitySlot: firstActivitySlot(), arrival: form.arrival });
      setItin(result.itinerary); setSources(result.sources || []); setStep('result');
    } catch { setStep('error'); }
  };
  ...
  const runGenerateLocalTaste = async () => {
    ...
    try {
      const { generateLocalTaste } = await import('./api');
      const r = await generateLocalTaste({ destination: form.dest });
      setLocalTasteData(r); setLocalTasteStep('result');
    } catch { setLocalTasteStep('error'); }
  };
  ...
  return (
    <div>
      <div style={{ display: 'flex', gap: 0, background: BRAND.navyCard, border: `1px solid ${BRAND.borderLight}`, borderRadius: 13, padding: 3, marginBottom: '1.1rem' }}>
        {ITABS.map(t => (
          <button key={t.id} onClick={() => setITab(t.id)}
            style={{ flex: 1, padding: '8px 8px', fontSize: 12, fontWeight: 500, borderRadius: 10, border: 'none', cursor: 'pointer', fontFamily: "'DM Sans',sans-serif", background: iTab === t.id ? BRAND.gradientBtn : 'transparent', color: iTab === t.id ? '#fff' : BRAND.textMuted }}>
            {t.label}
          </button>
        ))}
      </div>
      ...
      {iTab === 'planner' && (
        <div>
          {step === 'loading' && (
            <div style={{ textAlign: 'center', padding: '4rem 1.5rem' }}>
              <div style={S.spinner} />
              <div style={{ fontFamily: "'Poppins',sans-serif", fontSize: 16, fontWeight: 700, marginBottom: 8 }}>Building your itinerary…</div>
              <div style={{ fontSize: 13, color: BRAND.textMuted, lineHeight: 1.7 }}>🔍 Scanning top travel sources<br />📊 Ranking by ratings & reviews<br />🗓️ Scheduling from your arrival slot</div>
            </div>
          )}
          {step === 'error' && (
            <div style={{ textAlign: 'center', padding: '3rem 1.5rem' }}>
              <div style={{ fontSize: 40, marginBottom: 12 }}>😕</div>
              <div style={{ fontSize: 15, fontWeight: 600, marginBottom: 16 }}>Couldn't generate itinerary</div>
              <button style={{ ...S.btn, ...S.btnP, padding: '10px 24px' }} onClick={runGenerateItinerary}>Try Again</button>
            </div>
          )}
          {step === 'result' && itin && (
            <div style={{ paddingBottom: '2rem' }}>
              {/* Header */}
              <div style={{ background: 'linear-gradient(135deg,#1a1040,#0d1a3a)', borderRadius: 16, padding: '1.25rem 1.5rem', marginBottom: '1rem', border: `1px solid ${BRAND.border}`, position: 'relative', overflow: 'hidden' }}>
                <div style={{ position: 'absolute', top: -10, right: -10, width: 120, height: 120, borderRadius: '50%', background: 'radial-gradient(circle, rgba(123,97,255,0.15) 0%, transparent 70%)' }} />
                <div style={{ fontFamily: "'Poppins',sans-serif", fontSize: 17, fontWeight: 700, marginBottom: 4, background: BRAND.gradient, WebkitBackgroundClip: 'text', WebkitTextFillColor: 'transparent', backgroundClip: 'text' }}>{itin.headline || `${days}-Day ${form.dest} Itinerary`}</div>
                <div style={{ fontSize: 12, color: BRAND.textMuted, lineHeight: 1.6, marginBottom: 12 }}>{itin.summary}</div>
                <div style={{ display: 'flex', gap: 8, flexWrap: 'wrap', alignItems: 'center' }}>
                  <div style={{ background: 'rgba(255,255,255,0.07)', borderRadius: 8, padding: '4px 10px', fontSize: 11, color: BRAND.textMuted }}>✈️ Arrives {SLOT_LABELS[form.arrivalSlot]}</div>
                  <div style={{ background: 'rgba(255,255,255,0.07)', borderRadius: 8, padding: '4px 10px', fontSize: 11, color: BRAND.textMuted }}>🛫 Departs {SLOT_LABELS[form.departureSlot]}</div>
                  {itin.totalEstimatedCost && <div style={{ background: 'rgba(255,183,3,0.1)', borderRadius: 8, padding: '4px 10px', fontSize: 11, color: BRAND.yellow }}>💰 {itin.totalEstimatedCost}</div>}
                  <button style={{ marginLeft: 'auto', ...S.btn, fontSize: 12 }} onClick={() => { hasGenerated.current = false; runGenerateItinerary(); }}>↺ Redo</button>
                </div>
              </div>

              {itin.quickTips?.length > 0 && (
                <div style={{ ...S.card, marginBottom: '1rem', background: 'rgba(255,183,3,0.06)', border: `1px solid rgba(255,183,3,0.15)` }}>
                  <div style={{ fontSize: 11, fontWeight: 700, color: BRAND.yellow, textTransform: 'uppercase', letterSpacing: .4, marginBottom: 8 }}>💡 Quick Tips</div>
                  {itin.quickTips.map((tip, i) => <div key={i} style={{ fontSize: 13, color: BRAND.textMuted, lineHeight: 1.5, marginBottom: i < itin.quickTips.length - 1 ? 5 : 0 }}>· {tip}</div>)}
                </div>
              )}
              ...
              {(itin.days || []).map((d, dayIndex) => {
                ...
                return (
                  <div key={d.day} style={{ ...S.card, padding: 0, overflow: 'hidden', marginBottom: 14 }}>
                    <div style={{ background: 'linear-gradient(135deg,#1a1040,#0d1a3a)', padding: '12px 16px', display: 'flex', alignItems: 'center', gap: 10 }}>
                      <div style={{ background: 'rgba(123,97,255,0.2)', color: BRAND.purple, borderRadius: 8, padding: '3px 10px', fontSize: 11, fontWeight: 700, flexShrink: 0 }}>{dateLabel}</div>
                      <div style={{ flex: 1 }}>
                        <div style={{ fontSize: 14, fontWeight: 600, color: BRAND.text }}>{d.title}</div>
                        {d.theme && <div style={{ fontSize: 11, color: BRAND.textMuted, marginTop: 1 }}>{d.theme}</div>}
                      </div>
                      <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'flex-end', gap: 3, flexShrink: 0 }}>
                        {isArrivalDay && <span style={{ fontSize: 9, fontWeight: 700, padding: '2px 7px', borderRadius: 6, background: 'rgba(123,97,255,0.2)', color: BRAND.purple }}>✈️ Arrives {SLOT_LABELS[form.arrivalSlot]}</span>}
                        {isDepartureDay && <span style={{ fontSize: 9, fontWeight: 700, padding: '2px 7px', borderRadius: 6, background: 'rgba(255,106,174,0.15)', color: BRAND.pink }}>🛫 Departs {SLOT_LABELS[form.departureSlot]}</span>}
                        {d.estimatedCost && <div style={{ fontSize: 11, color: BRAND.textMuted }}>{d.estimatedCost}</div>}
                      </div>
                    </div>
                    <div style={{ padding: '10px 16px' }}>
                      {(d.activities || []).map((a, i) => (
                        <div key={i} style={{ display: 'flex', gap: 12, padding: '10px 0', borderBottom: i < d.activities.length - 1 ? `1px solid ${BRAND.borderLight}` : 'none' }}>
                          <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', flexShrink: 0, width: 14 }}>
                            <div style={{ width: 10, height: 10, borderRadius: '50%', background: a.mustDo ? BRAND.purple : BRAND.borderLight, marginTop: 4, flexShrink: 0, border: a.mustDo ? `2px solid rgba(123,97,255,0.3)` : 'none', boxSizing: 'border-box' }} />
                            {i < d.activities.length - 1 && <div style={{ width: 1, flex: 1, background: BRAND.borderLight, marginTop: 3 }} />}
                          </div>
                          <div style={{ flex: 1, minWidth: 0 }}>
                            <div style={{ display: 'flex', alignItems: 'flex-start', gap: 8, marginBottom: 3 }}>
                              <span style={{ fontSize: 11, color: BRAND.textDim, width: 58, flexShrink: 0, paddingTop: 2 }}>{a.time}</span>
                              <div style={{ flex: 1 }}>
                                <div style={{ display: 'flex', alignItems: 'center', gap: 6, flexWrap: 'wrap' }}>
                                  <span style={{ fontSize: 16 }}>{a.icon || TYPE_ICONS[a.type] || '📍'}</span>
                                  <span style={{ fontSize: 14, fontWeight: 600, color: BRAND.text }}>{a.name}</span>
                                  {a.mustDo && <span style={{ fontSize: 9, fontWeight: 700, padding: '2px 6px', borderRadius: 8, background: 'rgba(123,97,255,0.12)', color: BRAND.purple, textTransform: 'uppercase', letterSpacing: .3 }}>Must do</span>}
                                </div>
                                {a.note && <div style={{ fontSize: 12, color: BRAND.textMuted, marginTop: 3, lineHeight: 1.5 }}>{a.note}</div>}
                                <div style={{ display: 'flex', gap: 10, marginTop: 5, flexWrap: 'wrap' }}>
                                  {a.duration && <span style={{ fontSize: 11, color: BRAND.textDim }}>⏱ {a.duration}</span>}
                                  {a.cost && <span style={{ fontSize: 11, color: BRAND.textDim }}>💰 {a.cost}</span>}
                                  {a.rating && <span style={{ fontSize: 11, color: BRAND.yellow }}>{a.rating}</span>}
                                </div>
                              </div>
                            </div>
                          </div>
              ...
              {sources.length > 0 && (
                <div style={{ ...S.card, marginBottom: '1rem' }}>
                  <div style={{ fontSize: 11, fontWeight: 700, color: BRAND.textMuted, textTransform: 'uppercase', letterSpacing: .4, marginBottom: 10 }}>🔍 Researched from</div>
                  <div style={{ display: 'flex', gap: 6, flexWrap: 'wrap' }}>
                    {sources.map((s, i) => (
                      <a key={i} href={s.url} target="_blank" rel="noreferrer" style={{ fontSize: 11, color: BRAND.purple, background: 'rgba(123,97,255,0.1)', border: `1px solid ${BRAND.border}`, borderRadius: 8, padding: '4px 10px', textDecoration: 'none' }}>🔗 {s.title?.slice(0, 28) || new URL(s.url).hostname}</a>
                    ))}
                  </div>
                </div>
              )}
      ...
      {iTab === 'taste' && (
        <LocalTastePage destination={form.dest} isSolo={isSolo} autoData={localTasteData} autoStep={localTasteStep} onRetry={runGenerateLocalTaste} />
      )}
    </div>
  );
}
...
function TripChatbot({ trip, myNickname }) {
  ...
  const buildSystem = () => {
    const expLines = expenses.slice(0, 10).map(e => `• ${e.desc}: ₹${e.amount} — paid by ${e.paidBy}, split ${Array.isArray(e.split) ? e.split.length : memberNames.length} ways`).join('\n');
    if (isSolo) return `You are a friendly solo travel AI inside TripBae.\nTRIP: ${trip.groupName} | ${trip.destination} | ${new Date(trip.arrival).toDateString()} – ${new Date(trip.departure).toDateString()}\nBUDGET: ₹${trip.budget || 'not set'} | SPENT: ₹${Math.round(totalSpend)} | REMAINING: ₹${Math.round((trip.budget || 0) - totalSpend)}\nEXPENSES:\n${expLines || 'None yet.'}\nBe encouraging, friendly, practical. Keep answers under 150 words.`;
    return `You are a friendly AI inside TripBae, an AI travel companion app.\nTRIP: ${trip.groupName} | ${trip.destination} | ${new Date(trip.arrival).toDateString()} – ${new Date(trip.departure).toDateString()} | Members: ${memberNames.join(', ')}\nTOTAL SPENT: ₹${Math.round(totalSpend)}\nEXPENSES:\n${expLines || 'None yet.'}\nBe warm, concise, helpful. Use ₹. Keep answers under 150 words.`;
  };

  const SYSTEM = buildSystem();
  const SUGGESTIONS = isSolo ? ["Budget left?","What to do tomorrow?","Best street food?","Safety tips?"] : ["Who owes the most?","What to eat here?","Budget per person?","Trip summary?"];

  const [open, setOpen] = useState(false);
  const [messages, setMessages] = useState([{ id: 1, role: 'assistant', text: isSolo ? "Hey adventurer! 🎒 I'm your AI travel companion.\nAsk me about your budget, what to do next, or local tips!" : "Hey! I'm your TripBae AI ✈️\nAsk me anything — expenses, who owes what, or local tips!" }]);
  const [input, setInput] = useState('');
  const [loading, setLoading] = useState(false);
  const endRef = useRef(null);
  ...
  return (
    <>
      <style>{`@keyframes bounce{0%,100%{transform:translateY(0)}50%{transform:translateY(-5px)}} @keyframes fadeUp{from{opacity:0;transform:translateY(12px)}to{opacity:1;transform:translateY(0)}}`}</style>
      <button onClick={() => setOpen(v => !v)} style={{ position: 'fixed', bottom: 22, right: 18, width: 56, height: 56, borderRadius: '50%', background: BRAND.gradientBtn, border: 'none', boxShadow: '0 4px 24px rgba(123,97,255,0.4)', cursor: 'pointer', display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: 24, zIndex: 300, transition: 'transform .2s', transform: open ? 'scale(0.88)' : 'scale(1)' }}>{open ? '✕' : '🤖'}</button>
      {open && (
        <div style={{ position: 'fixed', bottom: 92, right: 14, width: 345, maxWidth: 'calc(100vw - 28px)', background: BRAND.navyMid, borderRadius: 20, boxShadow: '0 16px 60px rgba(0,0,0,0.5)', border: `1px solid ${BRAND.border}`, zIndex: 299, display: 'flex', flexDirection: 'column', maxHeight: '68vh', overflow: 'hidden', animation: 'fadeUp .2s ease-out' }}>
          <div style={{ background: 'linear-gradient(135deg,#1a1040,#0d1a3a)', borderRadius: '20px 20px 0 0', padding: '14px 18px', display: 'flex', alignItems: 'center', gap: 10, flexShrink: 0 }}>
            <div style={{ width: 38, height: 38, borderRadius: '50%', background: 'rgba(123,97,255,0.2)', display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: 20 }}>🤖</div>
            <div style={{ flex: 1 }}>
              <div style={{ fontFamily: "'Poppins',sans-serif", fontSize: 14, fontWeight: 700, background: BRAND.gradient, WebkitBackgroundClip: 'text', WebkitTextFillColor: 'transparent', backgroundClip: 'text' }}>TripBae AI</div>
              <div style={{ fontSize: 11, color: BRAND.textMuted }}>{trip.groupName} · {trip.destination}</div>
            </div>
          </div>
          <div style={{ flex: 1, overflowY: 'auto', padding: '12px 14px', display: 'flex', flexDirection: 'column', gap: 10 }}>
            {messages.map(m => (
              <div key={m.id} style={{ display: 'flex', flexDirection: m.role === 'user' ? 'row-reverse' : 'row', gap: 8, alignItems: 'flex-end' }}>
                {m.role === 'assistant' && <div style={{ width: 28, height: 28, borderRadius: '50%', background: 'rgba(123,97,255,0.12)', display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: 15, flexShrink: 0 }}>🤖</div>}
                <div style={{ maxWidth: '80%', padding: '9px 13px', borderRadius: 16, fontSize: 13, lineHeight: 1.55, background: m.role === 'user' ? BRAND.gradientBtn : BRAND.navyLight, color: BRAND.text, borderBottomRightRadius: m.role === 'user' ? 4 : 16, borderBottomLeftRadius: m.role === 'user' ? 16 : 4, whiteSpace: 'pre-wrap', wordBreak: 'break-word' }}>{m.text}</div>
              </div>
            ))}
            {loading && (
              <div style={{ display: 'flex', gap: 8, alignItems: 'flex-end' }}>
                <div style={{ width: 28, height: 28, borderRadius: '50%', background: 'rgba(123,97,255,0.12)', display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: 15 }}>🤖</div>
                <div style={{ padding: '10px 14px', background: BRAND.navyLight, borderRadius: 16, borderBottomLeftRadius: 4 }}>
                  <div style={{ display: 'flex', gap: 4, alignItems: 'center' }}>
                    {[0, 1, 2].map(i => <div key={i} style={{ width: 7, height: 7, borderRadius: '50%', background: BRAND.purple, animation: `bounce .9s ease-in-out ${i * 0.22}s infinite` }} />)}
                  </div>
                </div>
              </div>
            )}
          ...
          {messages.length <= 2 && !loading && (
            <div style={{ padding: '4px 14px 8px', display: 'flex', gap: 5, flexWrap: 'wrap', flexShrink: 0 }}>
              {SUGGESTIONS.map(s => (
                <button key={s} onClick={() => send(s)} style={{ ...S.btn, fontSize: 11, padding: '4px 10px', borderRadius: 16, background: 'rgba(123,97,255,0.1)', color: BRAND.purple, border: `1px solid ${BRAND.border}` }}>{s}</button>
              ))}
            </div>
          )}
          <div style={{ padding: '10px 14px 14px', borderTop: `1px solid ${BRAND.borderLight}`, display: 'flex', gap: 8, flexShrink: 0 }}>
            <input style={{ ...S.input, flex: 1, borderRadius: 22, padding: '9px 14px', fontSize: 13 }} value={input} onChange={e => setInput(e.target.value)} onKeyDown={e => e.key === 'Enter' && !loading && send()} placeholder="Ask your AI travel companion…" />
            <button onClick={() => send()} disabled={!input.trim() || loading} style={{ ...S.btn, ...S.btnP, borderRadius: '50%', width: 40, height: 40, padding: 0, justifyContent: 'center', flexShrink: 0, opacity: (!input.trim() || loading) ? 0.45 : 1 }}>➤</button>
          </div>
        </div>
      )}
    </>
...
function TripActionMenu({ trip, onMarkComplete, onDelete }) {
  ...
  return (
    <>
      {confirmDelete && <ConfirmDialog title="Delete Trip" message={`Delete "${trip.groupName}"? All data will be lost.`} confirmLabel="🗑️ Delete" confirmStyle="danger" onConfirm={() => { setConfirmDelete(false); onDelete(); }} onCancel={() => setConfirmDelete(false)} />}
      {confirmComplete && <ConfirmDialog title="Mark as Completed?" message={`"${trip.groupName}" will be moved to Past Trips.`} confirmLabel="✅ Mark Complete" confirmStyle="primary" onConfirm={() => { setConfirmComplete(false); onMarkComplete(); }} onCancel={() => setConfirmComplete(false)} />}
      <div style={{ position: 'relative' }}>
        <button onClick={() => setOpen(v => !v)} style={{ ...S.btn, padding: '5px 9px', fontSize: 15, color: BRAND.textMuted }}>⋯</button>
        {open && (
          <>
            <div style={{ position: 'fixed', inset: 0, zIndex: 198 }} onClick={() => setOpen(false)} />
            <div style={{ position: 'absolute', top: '110%', right: 0, background: BRAND.navyMid, border: `1px solid ${BRAND.borderLight}`, borderRadius: 12, boxShadow: '0 12px 40px rgba(0,0,0,0.5)', zIndex: 199, minWidth: 190, overflow: 'hidden' }}>
              {!trip.completed && <button onClick={() => { setOpen(false); setConfirmComplete(true); }} style={{ display: 'flex', alignItems: 'center', gap: 10, width: '100%', padding: '12px 16px', border: 'none', background: 'none', fontSize: 13, cursor: 'pointer', color: BRAND.success, fontFamily: "'DM Sans',sans-serif", borderBottom: `1px solid ${BRAND.borderLight}`, textAlign: 'left' }}>✅ Mark as Completed</button>}
              <button onClick={() => { setOpen(false); setConfirmDelete(true); }} style={{ display: 'flex', alignItems: 'center', gap: 10, width: '100%', padding: '12px 16px', border: 'none', background: 'none', fontSize: 13, cursor: 'pointer', color: BRAND.danger, fontFamily: "'DM Sans',sans-serif", textAlign: 'left' }}>🗑️ Delete Trip</button>
            </div>
          </>
        )}
      </div>
...
export default function App() {
  ...
  useEffect(() => {
    if (!authToken) return;
    setTripsLoading(true);
    getTrips().then(d => setTrips(d.trips || [])).catch(() => setTrips([])).finally(() => setTripsLoading(false));
  }, [authToken]);
  ...
  useEffect(() => {
    ...
    setTripLoading(true);
    import('./api').then(({ getTrip }) => {
      getTrip(activeTrip).then(d => {
        const localTrip = trips.find(x => x.id === activeTrip);
        setActiveTripData({ ...d.trip, _cachedItin: localTrip?._cachedItin || d.trip._cachedItin, _cachedTaste: localTrip?._cachedTaste || d.trip._cachedTaste });
        setMyNickname(d.myNickname);
      }).catch(() => {
        const t = trips.find(x => x.id === activeTrip);
        setActiveTripData(t || null);
  ...
  const handleAuth = async () => {
    ...
    try {
      const endpoint = authScreen === 'signup' ? '/auth/signup' : '/auth/login';
      const body = authScreen === 'signup' ? { name: authForm.name, email: authForm.email, password: authForm.password } : { email: authForm.email, password: authForm.password };
      const res = await fetch(`https://travelbae-backend.onrender.com${endpoint}`, { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify(body) });
      const data = await res.json();
      if (!res.ok) throw new Error(data.error || 'Something went wrong');
      localStorage.setItem('travelbae_token', data.token);
      setAuthToken(data.token);
  ...
  const handleCreateTrip = async (tripData) => {
    ...
    if (trip.destination) {
      import('./api').then(async ({ generateItinerary, generateLocalTaste }) => {
        const days = trip.arrival && trip.departure ? Math.max(1, Math.round((new Date(trip.departure) - new Date(trip.arrival)) / 86400000)) : 1;
        const arrivalIdx = SLOT_ORDER.indexOf(trip.arrivalSlot || 'morning');
        const firstSlot = SLOT_ORDER[Math.min(arrivalIdx + 1, SLOT_ORDER.length - 1)];
        try {
          const [itinResult, tasteResult] = await Promise.all([
            generateItinerary({ destination: trip.destination, days, budget: trip.budget || null, people: trip.people || 1, interests: ['🛕 Temples', '🍽️ Food', '🛍️ Shopping'], arrivalSlot: trip.arrivalSlot || 'morning', departureSlot: trip.departureSlot || 'morning', firstActivitySlot: firstSlot, arrival: trip.arrival }),
            generateLocalTaste({ destination: trip.destination }),
          ]);
          setTrips(ts => ts.map(t => t.id === trip.id ? { ...t, _cachedItin: itinResult, _cachedTaste: tasteResult } : t));
        } catch (e) { console.warn('Background itinerary generation failed:', e); }
      });
    }
  };
  ...
  const handleOpenTrip = (tripId) => { setActiveTrip(tripId); setTab('main'); };
  const handleShareCodeDismiss = () => { const id = newTripModal.id; setNewTripModal(null); setActiveTrip(id); setTab('main'); };

  const handleDeleteTrip = async (tripId) => {
    try { const { deleteTrip } = await import('./api'); await deleteTrip(tripId); } catch (err) { console.warn('Delete API error:', err.message); }
    setTrips(ts => ts.filter(t => t.id !== tripId));
    if (activeTrip === tripId) { setActiveTrip(null); setActiveTripData(null); }
  };

  const handleMarkComplete = async (tripId) => {
    try { const { updateTrip } = await import('./api'); await updateTrip(tripId, { completed: true }); } catch (err) { console.warn('Update API error:', err.message); }
    setTrips(ts => ts.map(t => t.id === tripId ? { ...t, completed: true } : t));
    if (activeTrip === tripId) { setActiveTrip(null); setActiveTripData(null); }
  };

  const handleMarkActive = async (tripId) => {
    try { const { updateTrip } = await import('./api'); await updateTrip(tripId, { completed: false }); } catch (err) { console.warn('Update API error:', err.message); }
    setTrips(ts => ts.map(t => t.id === tripId ? { ...t, completed: false } : t));
  };

  const groupTabs = [{ id: 'main', label: '💳 Split' }, { id: 'contacts', label: '📒 Contacts' }, { id: 'itinerary', label: '🗺️ Itinerary' }, { id: 'photos', label: '📸 Photos' }];
  const soloTabs = [{ id: 'main', label: '💰 Expenses' }, { id: 'contacts', label: '📒 Contacts' }, { id: 'itinerary', label: '🗺️ Itinerary' }];
  const tabs = isSolo ? soloTabs : groupTabs;

  /* ── AUTH SCREEN ── */
  if (!authToken) return (
    <div style={{ ...S.root, display: 'flex', alignItems: 'center', justifyContent: 'center', minHeight: '100vh', padding: '1.5rem', background: `radial-gradient(ellipse at top, #1a1040 0%, ${BRAND.navy} 60%)` }}>
      <style>{`@keyframes spin{to{transform:rotate(360deg)}} *{box-sizing:border-box;margin:0;padding:0}`}</style>
      {/* Background orbs */}
      <div style={{ position: 'fixed', top: -100, right: -100, width: 400, height: 400, borderRadius: '50%', background: 'radial-gradient(circle, rgba(123,97,255,0.08) 0%, transparent 70%)', pointerEvents: 'none' }} />
      <div style={{ position: 'fixed', bottom: -50, left: -80, width: 300, height: 300, borderRadius: '50%', background: 'radial-gradient(circle, rgba(255,106,174,0.06) 0%, transparent 70%)', pointerEvents: 'none' }} />

      <div style={{ width: '100%', maxWidth: 380 }}>
        {/* Logo */}
        <div style={{ textAlign: 'center', marginBottom: '2.5rem' }}>
          <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'center', gap: 10, marginBottom: 16 }}>
            <div style={{ width: 56, height: 56, background: 'rgba(123,97,255,0.12)', border: `1px solid ${BRAND.border}`, borderRadius: 16, display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
              <TripBaeLogo size={32} />
            </div>
          </div>
          <WordMark size="lg" />
          <div style={{ fontSize: 13, color: BRAND.textMuted, marginTop: 6, letterSpacing: .3 }}>YOUR AI TRAVEL COMPANION</div>
          <div style={{ display: 'flex', gap: 16, justifyContent: 'center', marginTop: 16, flexWrap: 'wrap' }}>
            {['AI Itinerary', 'Split Bills', 'Trip Support'].map(f => (
              <div key={f} style={{ fontSize: 11, color: BRAND.textDim, display: 'flex', alignItems: 'center', gap: 4 }}>
                <span style={{ width: 4, height: 4, borderRadius: '50%', background: BRAND.purple, display: 'inline-block' }} />{f}
              </div>
            ))}
          </div>
        </div>

        <div style={{ background: BRAND.navyCard, borderRadius: 20, padding: '1.75rem', boxShadow: '0 24px 80px rgba(0,0,0,0.4)', border: `1px solid ${BRAND.borderLight}` }}>
          <div style={{ display: 'flex', gap: 0, background: BRAND.navyLight, borderRadius: 12, padding: 3, marginBottom: '1.5rem', border: `1px solid ${BRAND.borderLight}` }}>
            {['login', 'signup'].map(s => (
              <button key={s} onClick={() => { setAuthScreen(s); setAuthError(''); }}
                style={{ flex: 1, padding: '9px', fontSize: 13, fontWeight: 500, borderRadius: 9, border: 'none', cursor: 'pointer', fontFamily: "'DM Sans',sans-serif", background: authScreen === s ? BRAND.gradientBtn : 'transparent', color: authScreen === s ? '#fff' : BRAND.textMuted, transition: 'all .2s', boxShadow: authScreen === s ? '0 2px 12px rgba(123,97,255,0.3)' : 'none' }}>
                {s === 'login' ? '🔑 Log In' : '✨ Sign Up'}
              </button>
            ))}
          </div>
          ...
          <label style={S.label}>Password</label>
          <input style={{ ...S.input, marginBottom: 10 }} type="password" value={authForm.password} onChange={e => setAuthForm(f => ({ ...f, password: e.target.value }))} onKeyDown={e => e.key === 'Enter' && handleAuth()} placeholder="Min 6 characters" />
          {authError && <div style={{ fontSize: 13, color: BRAND.danger, background: BRAND.dangerBg, border: `1px solid ${BRAND.dangerBorder}`, borderRadius: 10, padding: '9px 12px', marginBottom: 10 }}>⚠️ {authError}</div>}
          <button style={{ ...S.btn, ...S.btnP, width: '100%', justifyContent: 'center', padding: '12px', fontSize: 15, borderRadius: 12, marginTop: 4, opacity: authLoading ? 0.6 : 1 }} onClick={handleAuth} disabled={authLoading}>
            {authLoading ? 'Please wait…' : authScreen === 'login' ? '🔑 Log In' : '🚀 Create Account'}
          </button>
          <p style={{ fontSize: 11, color: BRAND.textDim, textAlign: 'center', marginTop: 14, lineHeight: 1.6 }}>
            Plan, split and explore — together. {authScreen === 'login' ? "Don't have an account? " : 'Already have an account? '}
            <span style={{ color: BRAND.purple, cursor: 'pointer', fontWeight: 600 }} onClick={() => setAuthScreen(authScreen === 'login' ? 'signup' : 'login')}>{authScreen === 'login' ? 'Sign up' : 'Log in'}</span>
          </p>
        </div>
      </div>
    </div>
  ...
  return (
    <div style={S.root}>
      ...
      <style>{`@keyframes spin{to{transform:rotate(360deg)}} @keyframes slideIn{from{opacity:0;transform:translateX(-6px)}to{opacity:1;transform:translateX(0)}} *{box-sizing:border-box;margin:0;padding:0} a{color:inherit;text-decoration:none} input,select,textarea{color-scheme:dark} input[type="date"]{color-scheme:dark}`}</style>
      ...
      <div style={S.topBar}>
        ...
        {activeTrip && activeTripData ? (
          <div style={{ marginLeft: 'auto', display: 'flex', alignItems: 'center', gap: 6 }}>
            <div style={{ display: 'flex', alignItems: 'center', gap: 5, background: isSolo ? 'rgba(255,106,174,0.1)' : 'rgba(123,97,255,0.1)', border: `1px solid ${isSolo ? 'rgba(255,106,174,0.25)' : BRAND.border}`, borderRadius: 20, padding: '5px 12px', fontSize: 12, color: isSolo ? BRAND.pink : BRAND.purple, fontWeight: 500, cursor: 'pointer' }} onClick={() => { setActiveTrip(null); setActiveTripData(null); }}>
              {activeTripData.emoji} {activeTripData.groupName}
              {isSolo && <span style={{ marginLeft: 4, fontSize: 10, fontWeight: 700, background: 'rgba(255,106,174,0.2)', borderRadius: 8, padding: '1px 6px' }}>Solo</span>}
            </div>
            <TripActionMenu trip={activeTripData} onMarkComplete={() => handleMarkComplete(activeTripData.id)} onDelete={() => handleDeleteTrip(activeTripData.id)} />
          </div>
        ) : (
          <div style={{ display: 'flex', alignItems: 'center', gap: 8, marginLeft: 'auto' }}>
            <button style={{ ...S.btn, fontSize: 12, padding: '5px 10px', color: BRAND.danger, borderColor: BRAND.dangerBorder }} onClick={handleLogout}>Log out</button>
          </div>
        )}
      </div>
      ...
      {activeTrip && activeTripData && (
        <div style={S.navTabs}>
          {tabs.map(t => (
            <button key={t.id} onClick={() => setTab(t.id)} style={{ ...S.navTab, ...(tab === t.id ? (isSolo ? S.soloNavTabActive : S.navTabActive) : {}) }}>{t.label}</button>
          ))}
        </div>
      )}
      ...
      <div style={S.page}>
        {!activeTrip && (
          tripsLoading
            ? <Spinner text="Loading your trips…" />
            : <HomePage trips={trips} onOpenTrip={handleOpenTrip} onCreateTrip={handleCreateTrip} onJoinTrip={handleJoinTrip} onDeleteTrip={handleDeleteTrip} onMarkComplete={handleMarkComplete} onMarkActive={handleMarkActive} />
        )}
        {activeTrip && (
          tripLoading || !activeTripData
            ? <Spinner text="Loading trip…" />
              ...
              <div style={{ animation: 'slideIn .2s ease-out' }}>
                {isSolo ? (
                  ...
                  <>
                    ...
                    {tab === 'contacts' && <ContactsPage trip={activeTripData} myNickname={myNickname} isSolo={false} />}
                    {tab === 'itinerary' && <ItineraryPage trip={activeTripData} />}
                    {tab === 'photos' && <div style={{ margin: '-1.25rem', marginBottom: '-6rem' }}><PhotosPage trip={activeTripData} myNickname={myNickname} /></div>}
                  </>
                )}
              </div>
            )
